...
main <- function (args) 
{
	...
	genoGwas2File   = "out/filtered-gwasp2-genotype.scores"
	phenoGwasp2File = "out/filtered-gwasp4-phenotype.scores"
	...
	markersSummaryTable ("out/", params$gwasModel,  "out/")
}
...
runPlinkGwas <- function (genotypeFile, phenotypeFile, model) 
{
	...
	if (model=="Naive") {
		cmm = paste0 ("plink --file out/filtered-plink-genotype --linear --adjust --pfilter 0.001 --pheno out/filtered-plink-phenotype.scores --all-pheno --allow-no-sex --out ", outFile)
		runCommand (cmm)
		...
		runCommand (paste0 ("plink --file out/filtered-plink-genotype --linear --assoc --adjust --pheno out/filtered-plink-phenotype.scores --all-pheno --allow-no-sex --out ", outFile))
	}
...
{
	inGenoPheno = "out/filtered-shesis-genopheno.scores"
	inMarkers   = "out/filtered-shesis-markernames.scores"
...
dataPreprocessing <- function (genotypeFile, phenotypeFile, params) 
{
	...
	outGenoFile  <- "out/filtered-gwasp4-genotype.scores"
	outPhenoFile <- "out/filtered-gwasp4-phenotype.scores"
	...
	msg (outPhenoFile)
	gwasp2plinkPhenotype  (outPhenoFile,"out/filtered-plink-phenotype.scores") 
	gwasp2tasselPhenotype (outPhenoFile,"out/filtered-tassel-phenotype.scores") 
	gwasp2shesisGenoPheno ("out/filtered-gwasp4-genotype.scores", "out/filtered-gwasp4-phenotype.scores") 
...
genotypeFiltering <- function (plinkFile, params) {
	...
	write.table (file="out/filtered-names-samples.scores", sampleNames, quote=F,row.names=F,col.names=F)
	markerNames = read.table (paste0(plinkFile,"-QC.map"))[,2]
	write.table (file="out/filtered-names-markers.scores", markerNames, quote=F,row.names=F,col.names=F)

	msg ("Reading filtered markers and individuals...")
	filteredMarkers <- read.table (file="out/filtered-names-markers.scores", stringsAsFactors=F)[,1]
	filteredSamples <- read.table (file="out/filtered-names-samples.scores", stringsAsFactors=F)[,1]
