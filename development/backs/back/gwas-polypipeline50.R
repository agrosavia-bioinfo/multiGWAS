...
args = c("in/config-test500-Naive-noFilters.config")
...
main <- function (args) 
{
	...
	mclapply (c("Gwasp", "Plink", "Shesis", "Tassel"), runGwasTool, params, mc.cores=4)
	#runGwaspGwas (params)
	#runPlinkGwas (params)
	#runShesisGwas (params)
	#runTasselGwas (params)

	markersSummaryTable ("out/", params$gwasModel, "out/", nBEST=5)
}

#-------------------------------------------------------------
#-------------------------------------------------------------
runGwasTool <- function (tool, params) {
	if (tool=="Gwasp")
		runGwaspGwas (params)
	else if (tool=="Plink")
		runPlinkGwas (params)
...
runGwaspGwas <- function (params) 
{
	...
	data2 = controlPopulationStructure (data1, params$gwasModel, data2)
...
runPlinkGwas <- function (params) 
{
	...
	if (model=="Naive") {
		...
		runCommand ("plink --file out/filtered-plink-genotype --make-bed --out out/tmp-plinkb", "log-Plink.log")
		runCommand ("plink2 --bfile out/tmp-plinkb --make-pgen --out out/tmp-plinkg", "log-Plink.log")
		runCommand ("plink2 --pfile out/tmp-plinkg --king-cutoff 0.177 --out out/tmp-plinkg", "log-Plink.log")
...
genotypeFiltering <- function (plinkFile, params) {
	...
	runCommand (sprintf ("plink --bfile %s-QC --recode tab --out %s-QC", plinkFile, plinkFile), "log-filtering.log")
...
#-------------------------------------------------------------
controlPopulationStructure <- function (data1, gwasModel, data2) 
{
	...
	}else if (gwasModel == "Kinship_PCs") {
		msg ("    >>>> With control using default Kinship and PCs=5 ")
		kinshipMatrix = NULL
		data2         = set.K (data1)
