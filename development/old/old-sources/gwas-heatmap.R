...
main <- function () {
	...
	snpId               <- "c2_21314" #SNP a visualizar
	createHeatmapForSNP ( genoFilenameACGT, genoFilenameNUMERIC, phenoFilename, snpId) 
}

#----------------------------------------------------------
# createHeatmapForSNP
#----------------------------------------------------------
createHeatmapForSNP <- function (genoFilenameACGT, genoFilenameNUMERIC, phenoFilename, snpId) {
	...
	outName = paste0 ("out-profileSNP-", snpId) 
	pdf (paste0(outName, ".pdf"), width=7, height=7)
	heatmap.2(genoxfenov4,dendrogram = "row",reorderfun=function(snpId, w) reorder(snpId, w, agglo.FUN = mean),Colv=FALSE,adjCol = c(NA,0),key=TRUE,srtCol=360,col=my_palette,cexCol = 1,lmat=lmat,lhei=lhei, lwid=lwid, extrafun=myplot)
	dev.off()
	png (paste0(outName, ".png"), width=7, height=7, units="in", res=72)
	heatmap.2(genoxfenov4,dendrogram = "row",reorderfun=function(snpId, w) reorder(snpId, w, agglo.FUN = mean),Colv=FALSE,adjCol = c(NA,0),key=TRUE,srtCol=360,col=my_palette,cexCol = 1,lmat=lmat,lhei=lhei, lwid=lwid, extrafun=myplot)
	dev.off()
}
