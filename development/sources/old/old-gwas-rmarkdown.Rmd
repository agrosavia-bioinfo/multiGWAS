...
```{r initParams, echo=FALSE, message=TRUE}
#<!-- <<initParams>> -->
print (params)
```

```{r markersSummaryTable, echo=FALSE, message=FALSE}
<<markersSummaryTable>>
#snpsTables = markersSummaryTable (inputDir, gwasType, "Structure", outputDir,  nBEST)
```

# Input parameters
```{r getConfigurationParameters, echo=FALSE, message=FALSE, warnings=TRUE}
<<writeConfigurationParameters>>
paramsDF = read.table (file=paste0(inputDir,"out-input-parameters.tbl"), header=T, sep="\t")
kable (paramsDF) %>%
