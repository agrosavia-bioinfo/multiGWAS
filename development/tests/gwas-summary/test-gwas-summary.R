...
markersManhattanPlots <- function (inputDir, gwasModel, commonBest, commonSign, snpTables, outputDir, nBest=8) {
	...
	for (filename in files) {
		...
		# Search for only best for package
		#commonSNPs = intersect (commonBest, gwasResults$SNP, 
...
markersVennDiagrams <- function (summaryTable, gwasModel, scoresType, outFile){
	...
	HEIGHT = 9
 	...
 	png (paste0 (outFile,".png"), width=WIDTH, height=HEIGHT, units="in", res=72)
	grid.draw(v0)
	dev.off()
