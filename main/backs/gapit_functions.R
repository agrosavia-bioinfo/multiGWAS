  ...
  function(Y = NULL,
    ...
    for (trait in 2:2) {
      ...
      while (!isDone) {
        ...
        if (iteration.output)
          name.of.trait2 = paste("Iteration_", theLoop, ".", name.of.trait, sep =
                                   "")
        myPrior = FarmCPU.Prior(
          GM = GM,
          P = P,
          Prior = Prior,
            ...
            if (length(Porder) > 1) {
              if (LD.wise & (length(Porder) > threshold)) {
                ...
                for (po in 2:max_Porder) {
                  if (min(abs(seqQTN.selected[Porder[po]] - seqQTN.selected[Porder_new])) >
                      step_bin) {
                    Po = Po + 1
        ...
        if (theLoop == 2 &&
            is.null(seqQTN) | length(seqQTN) == 0 && theLoop == 2) {
      ...
      print(
        paste(
          "-------------Blink finished successfully in",
          round(time.all, 2),
          "seconds!-----------------"
        )
      )
...
`Blink.LDRemoveBlock` <-
  function(GDneo = NULL,
    ...
    if (min(ncol(GDneo), nrow(GDneo)) < 201)
      bound = FALSE
    if (orientation == "col") {
      n = nrow(GDneo)
      if (bound) {
...
`Blink.SUB` <-
  function(GM = NULL,
    ...
    ##############################################################################
    if (is.null(GLM$P))
      return(NULL)  #P is required
    if (is.null(QTN))
      return(NULL)  #QTN is required
    ...
    if (ncol(GLM$P) != 1) {
      if (length(index) > 1) {
        if (method == "penalty")
          P.QTN = apply(GLM$P[, index], 2, max, na.rm = TRUE)
        if (method == "reward")
          P.QTN = apply(GLM$P[, index], 2, min, na.rm = TRUE)
        if (method == "mean")
          P.QTN = apply(GLM$P[, index], 2, mean, na.rm = TRUE)
        if (method == "median")
          P.QTN = apply(GLM$P[, index], 2, median, na.rm = TRUE)
        if (method == "onsite")
          P.QTN = GLM$P0[(length(GLM$P0) - nqtn + 1):length(GLM$P0)]
      } else{
        if (method == "penalty")
          P.QTN = max(GLM$P[, index], na.rm = TRUE)
        if (method == "reward")
          P.QTN = min(GLM$P[, index], na.rm = TRUE)
        if (method == "mean")
          P.QTN = mean(GLM$P[, index], na.rm = TRUE)
        if (method == "median")
          P.QTN = median(GLM$P[, index], median, na.rm = TRUE)
        if (method == "onsite")
          P.QTN = GLM$P0[(length(GLM$P0) - nqtn + 1):length(GLM$P0)]
      }
...
`GAPIT.Bread` <-
  function(Y = NULL,
    ...
    if (method == "FaST" | method == "SUPER" | method == "DC")
    {
      GWAS = NULL
      GPS = NULL
      if (!is.null(vg) & !is.null(vg) & is.null(delta))
        delta = ve / vg
      if (is.null(vg) & is.null(ve))
      {
        myFaSTREML = GAPIT.get.LL(
          pheno = matrix(Y[, -1], nrow(Y), 1),
          geno = NULL,
          snp.pool = as.matrix(GK[, -1]),
          X0 = as.matrix(cbind(matrix(1, nrow(
            CV
          ), 1), CV[, -1]))
        )
      ...
      mySUPERFaST = GAPIT.SUPER.FastMLM(
        ys = matrix(Y[, -1], nrow(Y), 1),
        X0 = as.matrix(cbind(matrix(1, nrow(
          CV
        ), 1), CV[, -1])),
        snp.pool = as.matrix(GK[-1]),
        xs = as.matrix(GD[GTindex, -1]),
        vg = vg,
        delta = delta,
        LD = LD,
        method = method
    ...
    if (method == "FarmCPU")
    {
      ...
      if (!exists('FarmCPU', mode = 'function'))
      ...
      maf = as.data.frame(cbind(gene_taxa, apply(cbind(
        .5 * ss / ns, 1 - .5 * ss / ns
      ), 1, min)))
...
`GAPIT.Burger` <-
  function(Y = NULL,
    ...
    if (!is.null(CV)) {
      ...
      theCV = as.matrix(cbind(matrix(1, nrow(CV), 1), CV[, -1])) #reseted by Jiabo ,CV frame is wrong,and not rmx taxa
...
`GAPIT.Bus` <-
  function(Y = NULL,
    ...
    if (method == "FarmCPU")
    {
        ...
        if (num_bins > 0)
        {
          for (i in 1:num_bins)
          {
            ...
            if (min(aim_order) < num_regwas)
            {
              ...
              aim_area[c((min(aim_order) - num_regwas):(max(aim_order) + num_regwas))] =
                TRUE
            }
      ...
      #print(dim(GWAS))
      #print(head(GWAS))
      system(paste("rmx -f FarmCPU.", taxa, ".GWAS.Results.csv", sep = ""))
      system(paste(
        "rmx -f FarmCPU.",
        taxa,
        ".Manhattan.Plot.Genomewise.pdf",
        sep = ""
      ))
      system(paste("rmx -f FarmCPU.", taxa, ".QQ-Plot.pdf", sep = ""))
    ...
    if (method == "MLMM")
    {
      ...
      if (ncol(KI) != nrow(GD))
        print("Please make sure dim of K equal number of GD !!")
                       ...
                       plot = TRUE)
{
  ...
  for (i in 1:length(chr.num)) {
    ...
    col.seg <-
      c(col.seg, col[round(col.index[[i]] * length(col) / maxbin.num)])
    if (plot)
      segments(
        pos.x[[i]],
        -width / 5 - band * (i - length(chr.num) - 1),
  ...
  if (!is.null(legend.max)) {
    if (legend.max < Maxbin.num) {
      if (!maxbin.num %in% legend.y) {
        ...
        legend.y.col <-
          c(legend.y[c(-1,-length(legend.y))], maxbin.num)
      } else{
  ...
  legend.col <-
    c("grey", col[round(legend.y.col * length(col) / maxbin.num)])
  if (plot)
    legend(
...
GAPIT.Circle.Manhatton.Plot <- function(Pmap,
                                        ...
                                        threshold.lty = 2,
                                        amplify = TRUE,
                                        # is that available for remark signal pch col
                                        chr.labels = NULL,
                                        signal.cex = 2,
                                        signal.pch = 8,
                                        signal.col = "red",
                                        ...
                                        memo = "")
{
  ...
  if (!is.null(memo) && memo != "")
    memo <- paste("_", memo, sep = "")
  if (length(taxa) == 0)
    taxa <- "Index"
  taxa <- paste(taxa, memo, sep = "")
  col = rep(c('#FF6A6A',    '#FAC863',  '#99C794',    '#6699CC',  '#C594C5'),
            ceiling(length(taxa) / 5))
  ...
  if ("d" %in% plot.type) {
    ...
    if (file.output) {
      if (file == "jpg")
        jpeg(
          paste("SNP_Density.", paste(taxa, collapse = "."), ".jpg", sep = ""),
          width = 9 * dpi,
          height = 7 * dpi,
          res = dpi,
          quality = 100
      ...
      if (file == "pdf")
        pdf(
          paste("GAPIT.", taxa, ".SNP_Density.Plot.pdf" , sep = ""),
          width = 9,
          height = 7
        )
      if (file == "tiff")
        tiff(
          paste("SNP_Density.", paste(taxa, collapse = "."), ".tiff", sep = ""),
          width = 9 * dpi,
          height = 7 * dpi,
          res = dpi
        )
  ...
  if (length(plot.type) != 1 | (!"d" %in% plot.type)) {
    ...
    if (!missing(band)) {
      band <- floor(band * (sum(sapply(
        pvalue.pos.list, max
      )) / 100))
    } else{
      band <- floor((sum(sapply(
        pvalue.pos.list, max
      )) / 100))
    }
    if (band == 0)
    ...
    for (i in 0:(Nchr - 1)) {
      if (i == 0) {
        ...
        ticks[i + 1] <-
          max(pvalue.posN) - floor(max(pvalue.pos.list[[i + 1]]) / 2)
      } else{
        #pvalue <- append(pvalue,rep(Inf,band),after=sum(Num[1:i])+i*band)
        pvalue.posN <-
          c(pvalue.posN, max(pvalue.posN) + band + pvalue.pos.list[[i + 1]])
        ticks[i + 1] <-
          max(pvalue.posN) - floor(max(pvalue.pos.list[[i + 1]]) / 2)
      }
    }
      ...
      if (!is.null(threshold)) {
        if (!is.null(signal.line)) {
          for (l in 1:R) {
            if (LOG10) {
              signal.line.index <-
                c(signal.line.index, which(pvalueT[, l] < min(threshold)))
            } else{
              signal.line.index <-
                c(signal.line.index, which(pvalueT[, l] > max(threshold)))
            }
          }
  ...
  if ("c" %in% plot.type)
  {
    if (file.output) {
      ...
      if (file == "pdf")
        pdf(
          paste("GAPIT.", taxa, ".Circular.Manhattan.Plot.pdf" , sep = ""),
          width = 10,
          height = 10
        )
      if (file == "tiff")
    ...
    if (cir.density) {
      plot(
        NULL,
        xlim = c(1.05 * (-RR - 4 * cir.chr.h), 1.1 * (RR + 4 * cir.chr.h)),
        ylim = c(1.05 * (-RR - 4 * cir.chr.h), 1.1 * (RR + 4 * cir.chr.h)),
        axes = FALSE,
        xlab = "",
        ylab = ""
      )
      ...
      plot(
        NULL,
        xlim = c(1.05 * (-RR - 4 * cir.chr.h), 1.05 * (RR + 4 * cir.chr.h)),
        ylim = c(1.05 * (-RR - 4 * cir.chr.h), 1.05 * (RR + 4 * cir.chr.h)),
        axes = FALSE,
        xlab = "",
        ylab = ""
      )
    ...
    if (!is.null(signal.line)) {
      if (!is.null(signal.line.index)) {
        ...
        Y1chr <- (RR) * cos(2 * pi * (signal.line.index - round(band / 2)) /
                              TotalN)
        X2chr <- (r) * sin(2 * pi * (signal.line.index - round(band / 2)) /
                             TotalN)
        Y2chr <- (r) * cos(2 * pi * (signal.line.index - round(band / 2)) /
                             TotalN)
        #print(signal.line)
    ...
    for (i in 1:R) {
      ...
      if (outward == TRUE) {
        if (cir.chr == TRUE) {
          ...
          for (k in 1:length(chr)) {
            if (k == 1) {
              polygon.index <-
                seq(round(band / 2) + 1,
                    -round(band / 2) + max(pvalue.posN.list[[1]]),
                    length = polygon.num)
              ...
              if (is.null(chr.den.col)) {
                polygon(
                  c(rev(X1chr), X2chr),
                  c(rev(Y1chr), Y2chr),
                  col = rep(colx, ceiling(
                    length(chr) / length(colx)
                  ))[k],
                  border = rep(colx, ceiling(
                    length(chr) / length(colx)
                  ))[k]
                )
              } else{
                if (cir.density) {
                  polygon(c(rev(X1chr), X2chr),
                          c(rev(Y1chr), Y2chr),
                          col = "grey",
                          border = "grey")
                } else{
              ...
              polygon.index <-
                seq(
                  1 + round(band / 2) + max(pvalue.posN.list[[k - 1]]),
                  -round(band / 2) + max(pvalue.posN.list[[k]]),
                  length = polygon.num
                )
              X1chr = (RR) * sin(2 * pi * (polygon.index) / TotalN)
              Y1chr = (RR) * cos(2 * pi * (polygon.index) / TotalN)
          ...
          if (cir.density) {
            segments(
              ...
              (RR + cir.chr.h) * sin(2 * pi * (pvalue.posN - round(band / 2)) /
                                       TotalN),
              (RR + cir.chr.h) * cos(2 * pi * (pvalue.posN - round(band / 2)) /
                                       TotalN),
        ...
        if (cir.legend == TRUE) {
          ...
          if (Max <= 1) {
            round.n = nchar(as.character(10 ^ (-ceiling(
              -log10(Max)
            )))) - 1
          } else{
            round.n = 1
        ...
        X = (Cpvalue + r + H * (i - 1) + cir.band * (i - 1)) * sin(2 * pi *
                                                                     (pvalue.posN - round(band / 2)) / TotalN)
        Y = (Cpvalue + r + H * (i - 1) + cir.band * (i - 1)) * cos(2 * pi *
                                                                     (pvalue.posN - round(band / 2)) / TotalN)
        # plot point in figure
        points(
          X[1:(length(X) - legend.bit)],
          Y[1:(length(Y) - legend.bit)],
          pch = 19,
          cex = cex[1],
        ...
        if (!is.null(threshold)) {
          if (sum(threshold != 0) == length(threshold)) {
            for (thr in 1:length(threshold)) {
              significantline1 = ifelse(LOG10, H * (-log10(threshold[thr])) / Max, H *
                                          (threshold[thr]) / Max)
              ...
              if (significantline1 < H) {
                ...
                #print("!!!!!")
                circle.plot(
                  myr = (significantline1 + r + H * (i - 1) + cir.band * (i - 1)),
                ...
                warning(
                  paste(
                    "No significant points for ",
                    taxa[i],
                    " pass the threshold level using threshold=",
                    threshold[thr],
                    "!",
                    sep = ""
                  )
        ...
        if (!is.null(threshold)) {
          if (sum(threshold != 0) == length(threshold)) {
            if (amplify == TRUE) {
              ...
              HX1 = (Cpvalue[p_amp.index] + r + H * (i - 1) + cir.band * (i - 1)) *
                sin(2 * pi * (pvalue.posN[p_amp.index] - round(band / 2)) / TotalN)
              HY1 = (Cpvalue[p_amp.index] + r + H * (i - 1) + cir.band * (i - 1)) *
                cos(2 * pi * (pvalue.posN[p_amp.index] - round(band / 2)) / TotalN)
              ...
              for (ll in 1:length(threshold)) {
                if (ll == 1) {
                  ...
                  p_amp.index <- which(Cpvalue >= significantline1)
                  HX1 = (Cpvalue[p_amp.index] + r + H * (i - 1) + cir.band * (i -
                                                                                1)) * sin(2 * pi * (pvalue.posN[p_amp.index] - round(band / 2)) / TotalN)
                  HY1 = (Cpvalue[p_amp.index] + r + H * (i - 1) + cir.band * (i -
                                                                                1)) * cos(2 * pi * (pvalue.posN[p_amp.index] - round(band / 2)) / TotalN)
                } else{
                  if (LOG10) {
                  ...
                  p_amp.index <-
                    which(Cpvalue >= significantline1 & Cpvalue < significantline0)
                  HX1 = (Cpvalue[p_amp.index] + r + H * (i - 1) + cir.band * (i -
                                                                                1)) * sin(2 * pi * (pvalue.posN[p_amp.index] - round(band / 2)) / TotalN)
                  HY1 = (Cpvalue[p_amp.index] + r + H * (i - 1) + cir.band * (i -
                                                                                1)) * cos(2 * pi * (pvalue.posN[p_amp.index] - round(band / 2)) / TotalN)
                ...
                if (is.null(signal.col)) {
                  ...
                  points(
                    ...
                    HY1,
                    pch = signal.pch,
                    cex = signal.cex[ll] * cex[1],
                    col = rep(rep(colx, N[i]), add[[i]])[p_amp.index]
                  )
                } else{
                  # print(signal.pch)
                  points(
        ...
        if (cir.chr == TRUE) {
          ticks1 = 1.07 * (RR + cir.chr.h) * sin(2 * pi * (ticks - round(band / 2)) /
                                                   TotalN)
          ticks2 = 1.07 * (RR + cir.chr.h) * cos(2 * pi * (ticks - round(band /
                                                                           2)) / TotalN)
      ...
      if (outward == FALSE) {
        if (cir.chr == TRUE) {
          ...
          for (k in 1:length(chr)) {
            if (k == 1) {
              ...
              X2chr = (2 * cir.band + RR + cir.chr.h) * sin(2 * pi * (polygon.index) /
                                                              TotalN)
              Y2chr = (2 * cir.band + RR + cir.chr.h) * cos(2 * pi * (polygon.index) /
                                                              TotalN)
              if (is.null(chr.den.col)) {
                polygon(
                  c(rev(X1chr), X2chr),
          ...
          if (cir.density) {
            segments(
              ...
              (2 * cir.band + RR) * cos(2 * pi * (pvalue.posN - round(band / 2)) /
                                          TotalN),
              (2 * cir.band + RR + cir.chr.h) * sin(2 * pi * (pvalue.posN - round(band /
                                                                                    2)) / TotalN),
              (2 * cir.band + RR + cir.chr.h) * cos(2 * pi * (pvalue.posN - round(band /
                                                                                    2)) / TotalN),
              col = density.list$den.col,
              lwd = 0.1
            )
        ...
        X = (-Cpvalue + r + H * i + cir.band * (i - 1)) * sin(2 * pi * (pvalue.posN -
                                                                          round(band / 2)) / TotalN)
        Y = (-Cpvalue + r + H * i + cir.band * (i - 1)) * cos(2 * pi * (pvalue.posN -
                                                                          round(band / 2)) / TotalN)
          ...
          if (sum(threshold != 0) == length(threshold)) {
            for (thr in 1:length(threshold)) {
              ...
              if (significantline1 < H) {
                #lines(s1X,s1Y,type="l",col=threshold.col,lwd=threshold.col,lty=threshold.lty)
                circle.plot(
                  myr = (-significantline1 + r + H * i + cir.band * (i - 1)),
                  col = threshold.col[thr],
                  lwd = threshold.lwd[thr],
                  lty = threshold.lty[thr]
                )
            ...
            if (amplify == TRUE) {
              ...
              p_amp.index <- which(Cpvalue >= significantline1)
              HX1 = (-Cpvalue[p_amp.index] + r + H * i + cir.band * (i - 1)) *
                sin(2 * pi * (pvalue.posN[p_amp.index] - round(band / 2)) / TotalN)
              HY1 = (-Cpvalue[p_amp.index] + r + H * i + cir.band * (i - 1)) *
                cos(2 * pi * (pvalue.posN[p_amp.index] - round(band / 2)) / TotalN)
        ...
        if (cir.chr == TRUE) {
          ticks1 = 1.1 * (2 * cir.band + RR) * sin(2 * pi * (ticks - round(band /
                                                                             2)) / TotalN)
          ticks2 = 1.1 * (2 * cir.band + RR) * cos(2 * pi * (ticks - round(band /
                                                                             2)) / TotalN)
          if (is.null(chr.labels)) {
            for (i in 1:(length(ticks) - 1)) {
              angle = 360 * (1 - (ticks - round(band / 2))[i] / TotalN)
          ...
          ticks1 = 1.0 * (RR + cir.band) * sin(2 * pi * (ticks - round(band / 2)) /
                                                 TotalN)
          ticks2 = 1.0 * (RR + cir.band) * cos(2 * pi * (ticks - round(band /
                                                                         2)) / TotalN)
          if (is.null(chr.labels)) {
            for (i in 1:length(ticks)) {
  ...
  if ("q" %in% plot.type) {
    ...
    if (multracks) {
      ...
      for (i in 1:R) {
        ...
        if (conf.int)
          polygon(
            c(log.Quantiles[index], log.Quantiles),
            c(-log10(c05)[index], -log10(c95)),
            col = conf.int.col,
            border = conf.int.col
          )
          ...
          if (sum(threshold != 0) == length(threshold)) {
            ...
            if (amplify == TRUE) {
              ...
              if (length(thre.index) != 0) {
                #cover the points that exceed the threshold with the color "white"
                points(
                  log.Quantiles[thre.index],
                  log.P.values[thre.index],
                  col = "white",
                  pch = 19,
                  cex = cex[3]
                )
      ...
      if (R > 1) {
        #qq_col=rainbow(R)
        qq_col = rep(c('#FF6A6A',    '#FAC863',  '#99C794',    '#6699CC',  '#C594C5'),
                     ceiling(R / 5))
        ...
        YlimMax <-
          max(floor(max(max(
            ...
            -log10(c95)
          )) + 1),-log10(min(Pmap.min[Pmap.min > 0])))
        plot(
          NULL,
          xlim = c(0, floor(max(
            log.Quantiles
        ...
        axis(
          2,
          at = seq(0, floor(YlimMax + 1), ceiling((YlimMax + 1) / 10)),
          labels = seq(0, floor((YlimMax + 1)), ceiling((YlimMax + 1) / 10)),
          cex.axis = cex.axis
        )
      ...
      for (i in 1:R) {
        ...
        if (file.output) {
          ...
          if (file == "pdf")
            pdf(
              paste("QQplot.", taxa[i], ".pdf", sep = ""),
              width = 5.5,
              height = 5.5
            )
          if (file == "tiff")
            tiff(
              paste("QQplot.", taxa[i], ".tiff", sep = ""),
              width = 5.5 * dpi,
              height = 5.5 * dpi,
              res = dpi
            )
...
`GAPIT.Compression.Visualization` <-
  function(Compression = Compression,
    ...
    label.comp = paste(c("Cluster method: ", "Group method: ", "Group number: "),
                       Compression.best[c(1:3)],
                       sep = "")
    ...
    if (length(Compression) == 6)
      return() #For to exit if only one row
    ...
    if (length(unique(Compression[, 3])) > 1)
    {
      ...
      color.vector.basic <-
        c("red",
          ...
          "cadetblue",
          "orange")
      color.vector.addition <-
        setdiff(c(colors()[grep("red", colors())], colors()[grep("blue", colors())]), color.vector.basic)
      color.vector.addition.mixed <-
        sample(color.vector.addition, max(0, ((length(unique(Compression[, 1])) * length(unique(Compression[, 2]))) -
                                                length(color.vector.basic)
        )))
      ...
      line.vector <-
        rep(1:(length(unique(Compression[, 1])) * length(unique(Compression[, 2]))))
      ...
      for (i in 1:length(unique(Compression[, 1]))) {
        for (j in 1:length(unique(Compression[, 2]))) {
          if ((i == 1) & (j == 1)) {
            Compression.subset <-
              Compression[which((Compression[, 1] == as.character(unique(
                Compression[, 1]
              )[i])) &
                (Compression[, 2] == as.character(unique(
                  Compression[, 2]
                )[j]))),]
            ...
            plot(
              ...
              pch = 30,
              lty = line.vector[i],
              ylim = c(min(as.numeric(
                Compression[, 4]
              )), max(as.numeric(
                Compression[, 4]
              ))),
              xlim = c(min(as.numeric(
                Compression[, 3]
              )), max(as.numeric(
                Compression[, 3]
              ))),
              ...
              ylab = "-2Log Likelihoood",
              lwd = 1
            )
            label = paste(c(as.character(unique(
              Compression[, 1]
            ))[k], " ", as.character(unique(
              Compression[, 2]
            ))[j]), collapse = "")
          }
          ...
          if ((i != 1) | (j != 1)) {
            ...
            label = c(label, paste(c(
              as.character(unique(Compression[, 1]))[i], " ", as.character(unique(Compression[, 2]))[j]
            ), collapse = ""))
          }
        }
      }
      ...
      legend.col = 1 + floor(length(unique(Compression[, 1])) * length(unique(Compression[, 2])) /
                               20)
      ...
      #print("h2")
      heritablilty.vector <-
        as.numeric(Compression[, 5]) / (as.numeric(Compression[, 5]) + as.numeric(Compression[, 6]))
      totalVariance.vector <-
        as.numeric(as.numeric(Compression[, 5]) + as.numeric(Compression[, 6]))
      Compression.h2 <-
        cbind(Compression, heritablilty.vector, totalVariance.vector)
      ...
      for (i in 1:length(unique(Compression.h2[, 1]))) {
        for (j in 1:length(unique(Compression.h2[, 2]))) {
          if ((i == 1) & (j == 1)) {
            Compression.subset <-
              Compression.h2[which((Compression.h2[, 1] == as.character(unique(
                Compression.h2[, 1]
              )[i])) &
                (Compression.h2[, 2] == as.character(unique(
                  Compression.h2[, 2]
                )[j]))),]
            x <- as.numeric(Compression.subset[, 3])
    ...
    if (length(unique(Compression[, 3])) == 1 &
        length(unique(Compression[, 1])) * length(unique(Compression[, 2])) > 1)
    {
      #Graph the compression with only one group
      pdf(
        paste(
          "GAPIT.Compression.single.group.",
          name.of.trait,
          ".pdf",
          sep = ""
        ),
...
`GAPIT.Create.Indicator` <-
  function(xs, SNP.impute = "Major") {
    ...
    frequ <- NULL
    for (i in 1:length(unique(xs.temp)))
      frequ <- c(frequ, length(which(xs == unique(xs)[i])))
          ...
          if (!is.null(xs))  {
            ...
            for (i in loopStart:mloop) {
              ...
              if (normalCase)
              {
                ...
                #iXX <- try(solve(XX))
                #if(inherits(iXX, "try-error")) iXX <- MASS::ginv(crossprod(Xt, Xt))
                ...
                if (i == 0 & file == file.from & frag == 1 & !is.null(K))
                {
                  ...
                  if (inherits(C11, "try-error"))
                    C11 = vgs * MASS::ginv(crossprod(Xt, Xt))
                  ...
                  if (inherits(term.1, "try-error"))
                    term.1 = MASS::ginv(term.0 + Kinv / vgs)
            ...
            if (!fullGD)
            {
              ...
              #rmx(dfs,stats,ps,nobs,maf,GI)   #This cause problem on return
              #gc()
            }
...
`GAPIT.FDR.TypeI` <-
  function(WS = c(1e0, 1e3, 1e4, 1e5),
    ...
    for (theWS in 1:length(WS)) {
      wsws = WS[theWS]
      qtn.pool = ceiling((as.numeric(GWAS[seqQTN, 2]) * MaxBP + as.numeric(GWAS[seqQTN, 3])) /
                           (2 * wsws))
      bonf.pool = ceiling((GWAS[total.index, 2] * MaxBP + GWAS[total.index, 3]) /
                            (2 * wsws))
    ...
    for (j in 1:ncol(cutoff)) {
      ...
      for (theWS in 1:length(WS)) {
        ...
        wsws = WS[theWS]
        qtn.pool = ceiling((GWAS[seqQTN, 2] * MaxBP + GWAS[seqQTN, 3]) /
                             (2 * wsws))
        bonf.pool = ceiling((GWAS[p.index, 2] * MaxBP + GWAS[p.index, 3]) /
                              (2 * wsws))
        qtn.number = length(levels(factor(bonf.pool[bonf.pool %in% qtn.pool])))
        false.number = length(levels(factor(bonf.pool[!(bonf.pool %in%
                                                          qtn.pool)])))
        if (theWS == 1) {
          final = false.number
    ...
    if (method == "optimum" & optimumable) {
      ...
      for (bin in b) {
        for (inc in s) {
          ...
          myBurger = FarmCPU.Burger(Y = Y[, 1:2],
                                    CV = CV,
                                    GK = GK)
...
`FarmCPU.GLM` <-
  function(Y = NULL,
    ...
    ##############################################################################
    if (is.null(Y))
      return(NULL)  #Y is required
    if (is.null(GDP) &
        is.null(CV))
      return(NULL)  #Need to have either genotype of CV
    ...
    if (package == "lm") {
      ...
      for (i in 1:nrow(GDP)) {
        ...
        P[i, c((nf + 2):(2 * nf + 2))] = lmcoefOnly[(2 * nf + 3):(3 *
                                                                    nf + 3)]
        P[i, c((2 * nf + 3):(3 * nf + 3))] = lmcoefOnly[(nf + 2):(2 *
                                                                    nf + 2)]
        P[i, c((3 * nf + 4):(4 * nf + 4))] = lmcoefOnly[(3 * nf + 4):(4 *
                                                                        nf + 4)]
...
`FarmCPU.LM.Parallel` <-
  function(y,
    ...
    n = N
    if (nd > n)
      nd = n #handler of samples less than nd
    k = 1 #number of genetic effect: 1 and 2 for A and AD respectively
    if (model == "AD")
      k = 2
    ...
    P <- sfApply(x, direction, function(x) {
      ...
      if (model == "AD") {
        ...
        if (is.na(r))
          r = 1
        if (r <= threshold)
          x = cbind(x, d) # having both a and d as marker effects
      }
      ...
      if (abs(r) > threshold & model == "AD") {
        beta <-
          crossprod(iXX[-(q0 + k), -(q0 + k)], rhs) #the last one (d) dose not count
        df = n - q0 - 1
      } else{
        beta <- crossprod(iXX, rhs)   #both a and d go in
        df = n - q0 - 2
      ...
      if (model == "A")
        df = n - q0 - 1 #change it back for model A
...
#)#end of cmpfun(
`FarmCPU.Pred` <- function(pred = NULL,
                           ypred = NULL,
                           name.of.trait = "") {
  ...
  if (!is.null(pred)) {
    ...
    write.table(
      pred,
      paste("FarmCPU.", name.of.trait, ".Pred.csv", sep = ""),
      quote = FALSE,
      sep = ",",
      row.names = FALSE,
      col.names = TRUE
    ...
    pdf(
      paste("FarmCPU.", name.of.trait, ".Pred.pdf" , sep = ""),
      width = 5,
      height = 5
    )
    par(mar = c(5, 6, 5, 3))
    ...
    plot(
      ...
      las = 1,
      bty = 'n',
      xlim = c(floor(min(pred[, 2], na.rm = T)), ceiling(max(pred[, 2], na.rm =
                                                               T)) * 1.2),
      ylim = c(floor(min(pred[, 3], na.rm = T)), ceiling(max(pred[, 3], na.rm =
                                                               T)) * 1.2),
      xaxs = "i",
      yaxs = "i"
    )
    ...
    text(
      max(pred[, 2], na.rm = T) * 1,
      max(pred[, 3], na.rm = T) * 1,
      paste("R^2=", cor),
      col = "forestgreen",
      cex = 1,
      pos = 3
  ...
  if (!is.null(ypred)) {
    ...
    write.table(
      ypred,
      paste("FarmCPU.", name.of.trait, ".unknownY.Pred.csv", sep = ""),
      quote = FALSE,
      sep = ",",
      row.names = FALSE,
      col.names = TRUE
    ...
    if (length(ypredrna) != 0) {
      pdf(
        paste("FarmCPU.", name.of.trait, ".unknownY.Pred.pdf" , sep = ""),
        width = 5,
        height = 5
      )
      par(mar = c(5, 6, 5, 3))
      ...
      plot(
        ...
        las = 1,
        bty = 'n',
        xlim = c(floor(min(pred[, 2], na.rm = T)), ceiling(max(
          ypred[, 2], na.rm = T
        )) * 1.2),
        ylim = c(floor(min(pred[, 3], na.rm = T)), ceiling(max(
          ypred[, 3], na.rm = T
        )) * 1.2),
        xaxs = "i",
        yaxs = "i"
      ...
      text(
        max(ypred[, 2], na.rm = T) * 1,
        max(ypred[, 3], na.rm = T) * 1,
        paste("R^2=", ycor),
        col = "forestgreen",
        cex = 1,
        pos = 3
...
`FarmCPU.Prior` <-
  function(GM,
    ...
    if (is.null(Prior) & kinship.algorithm != "FARM-CPU")
      return(P)
    if (is.null(Prior) & is.null(P))
      return(P)
...
`FarmCPU` <-
  function(Y = NULL,
    ...
    print("FarmCPU Started...")
    if (ncol(Y) > 2)
      stop("FarmCPU only accept single phenotype, please specify a column, like myY[,c(1,3)]")
    #Set orientation
    ...
    for (trait in 2:ncol(Y))  {
      ...
      #print(head(CV1))
      if (length(seqTaxa) < 1)
        stop("FarmCPU stoped as no data in Y")
      ...
      while (!isDone) {
        ...
        if (iteration.output)
          name.of.trait2 = paste("Iteration_", spacer, theLoop, ".", name.of.trait, sep =
                                   "")
        if (method.bin == "NONE")
          maxLoop = 1 #force to exit for GLM model
        ...
        if (theLoop == 2 && is.null(seqQTN)) {
          ...
          #print(pred)
          if (!is.null(pred))
            pred = cbind(Y1, myGLM$Pred) #Need to be consistant to CMLM
          #print(pred)
          p.GLM = GWAS[, 4]
          p.GLM.log = -log10(quantile(p.GLM, na.rm = TRUE, 0.05))
          #set.seed(666)
          ...
          if (file.output) {
            if (npc != 0) {
              ...
              write.csv(
                betapc,
                paste("FarmCPU.", name.of.trait2, ".CVeffect.csv", sep = ""),
                quote = F,
                row.names = FALSE
              )
            }
        ...
        isDone = ((theLoop >= maxLoop) |
                    (theConverge >= converge)  | circle)
        ...
        if (isDone | iteration.output) {
          ...
          if (!is.null(pred))
            pred = cbind(Y1, myGLM$Pred) #Need to be consistant to CMLM
          if (!is.null(PredictY))
            ypred = cbind(PredictYt, PredictY) #Need to be consistant to CMLM
          #P=myGLM$P[,ncol(myGLM$P)-shift]
...
`FarmCPU.Remove` <-
  function(GDP = NULL,
    ...
    if (orientation == "col") {
      ...
      if (is.big.matrix(GDP)) {
        x = t(as.matrix(deepcopy(
          GDP, rows = seqQTN, cols = index
        )))
      } else{
  ...
  function(GD = NULL,
    ...
    for (i in 1:theRep) {
      ...
      myFarmCPU = FarmCPU(
        ...
        file.output = FALSE,
        method.bin = "optimum",
        #options are "static" and "optimum", default is static and this gives the fastest speed. If you want to use random model to optimize possible QTNs selection, use method.bin="optimum"
        maxLoop = 1,
        #maxLoop is used to set the maximum iterations you want
        iteration.output = TRUE,
        #iteration.output=TRUE means to output results of every iteration
      )
    ...
    write.table(
      pvalue.final,
      paste("FarmCPU.p.threshold.optimize.", trait, ".txt", sep = ""),
      sep = "\t",
      col.names = F,
      quote = F,
      row.names = F
  ...
  function(file.path = NULL,
    ...
    if (genoFormat == "hapmap") {
      ...
      if (frag == 1) {
        ...
        G <-
          try(read.delim(
            paste(file.path, file.G, file, ".", file.Ext.G, sep = ""),
            head = FALSE,
            skip = skip.1,
      ...
      if (!is.null(LD.chromosome) & !is.null(hm$GD)) {
        index = (G[, 3] == LD.chromosome[1]) &
          abs((as.numeric(G[, 4]) - as.numeric(LD.location[1])) < (as.numeric(LD.range[1]) /
                                                                     2))
        GLD = G[index, ]
      } else{
        GLD = NULL
    ...
    if (genoFormat == "EMMA") {
      ...
      GD.temp <-
        try(read.table(
          paste(file.path, file.GD, file, ".", file.Ext.GD, sep = ""),
          head = TRUE,
          nrows = 1
        ), silent = TRUE)
      ...
      GI <-
        try(read.table(
          paste(file.path, file.GM, file, ".", file.Ext.GM, sep = ""),
          head = TRUE,
          skip = skip.1,
          nrows = file.fragment
        ) ,
...
`GAPIT.GS.Visualization` <-
  function(gsBLUP = gsBLUP,
    ...
    if (BLUP[1] != "NaN")
    {
      ...
      if ((length(distinct.BLUP) < 2)  |
          (length(distinct.PEV) < 2))
      ...
      KG <-
        (tapply(as.numeric(value), list(Position.BLUP, Position.PEV), sum))
      ...
      pdf(paste("GAPIT.", name.of.trait, ".GPS.BLUPvsPEV", ".pdf", sep = ""),
          width = 9)
...
`GAPIT.Genotype.View` <-
  function(myGI = NULL,
    ...
    if (is.null(myGI)) {
      stop("Validation Invalid. Please select read valid Genotype flies  !")
    }
    ...
    hist(
      ...
      main = "",
      ylab = ylab.ind,
      xlab = "Heterozygosity of individuals"
    )
    hist(
      het.snp,
      col = "gray",
      main = "",
      ylab = ylab.snp,
      xlab = "Heterozygosity of markers"
    )
    dev.off()
    rmx(X, H, het.ind, het.snp) #Feree memory
    myFig21 <- myGI
    ...
    {
      k <-
        as.numeric(as.matrix(myFig2[i + 1, 3])) - as.numeric(as.matrix(myFig2[i, 3]))
      if (k > 0) {
        subResult[i] <- k
      }
      else{
    ...
    pdf("GAPIT.Marker.Density.pdf",
        width = 10,
        height = 6)
    par(mar = c(5, 5, 4, 5) + 0.1)
    hist(
      as.numeric(as.matrix(results[, 4])),
      xlab = "Density",
    ...
    for (j in 1:(km - 1))
    {
      for (k in (j + 1):km)
      {
        save_Density_Cor[countSDC, 1] <-
          abs(as.numeric(as.matrix(results3_100[k, 3])) - as.numeric(as.matrix(results3_100[j, 3])))
        ...
        save_Density_Cor[countSDC, 3] <-
          cor(myGD3_100[, j], myGD3_100[, k]) %*% cor(myGD3_100[, j], myGD3_100[, k])
        save_Density_Cor_name[countSDC, 1] <-
          paste(results3_100[j, 1], "::::", results3_100[k, 1], seq = "")
        countSDC <- countSDC + 1
      }
    }
    ...
    plot(
      as.matrix(result3_3[, 1]),
      as.matrix(result3_3[, 3]),
      bg = "dimgray",
      xlab = "Distance",
      ylab = "R Square",
      pch = 1,
...
`GAPIT.HapMap` <-
  function(G,
    ...
    if (!Create.indicator) {
      print(paste(
        "Succesfuly finished converting HapMap which has bits of ",
        bit,
        sep = ""
      ))
    }
...
`GAPIT.ID` <-
  function(DP = NULL,
    ...
    if (is.null(DP) & is.null(IC))
      #inputdata is other method result
    {
      GWAS = RS
      GI = RS[, 1:3]
      GI = GI[order(GI[, 2]), ]
      ...
      if (byPass)
      {
        ...
        if (DP$file.output &
            !is.null(SS$Compression) &
            !is.na(SS$Compression[1, 6]))
          GAPIT.Compression.Visualization(
      ...
      if (!is.null(IC$GD) & DP$SNP.test)
      {
        ...
        # print(head(GWAS))
        if (all.equal(as.character(DP$chor_taxa), as.character(unique(sort(
          as.numeric(as.matrix(GWAS[, 2]))
        )))) != TRUE)
        {
        ...
        if (DP$file.output)
          myROC = GAPIT.ROC(
            ...
            Vp = var(as.matrix(IC$Y[, 2])),
            trait = DP$name.of.trait
          )
        print("ROC table and plot created")
        ...
        if (DP$file.output) {
          if (ncol(GI) > 1)
          {
            new_GI = merge(PWIP$PWIP, GI[, c("SNP", "effect")], by.x = "SNP", by.y =
                             "SNP")
          } else{
            new_GI = GI
          }
          ...
          if (all.equal(as.character(DP$chor_taxa), as.character(sort(unique(
            as.numeric(as.matrix(new_GI[, 2]))
          )))) != TRUE)
          {
            # print("@@@")
          ...
          write.table(
            new_GI,
            paste("GAPIT.", DP$name.of.trait, ".GWAS.Results.csv", sep = ""),
            quote = FALSE,
            sep = ",",
            row.names = FALSE,
            col.names = TRUE
...
`GAPIT.Interactive.GS` <-
    ...
    #
  {
    ...
    if ("gBLUP" %in% model_store)
      method_store = append(method_store, "MLM")
    if ("cBLUP" %in% model_store)
      method_store = append(method_store, "CMLM")
    if ("sBLUP" %in% model_store)
      method_store = append(method_store, "SUPER")
    index = c("gBLUP", "cBLUP", "sBLUP") %in% model_store
    no_model = c("gBLUP", "cBLUP", "sBLUP")[!index]
    gs_store = NULL
    for (i in 1:n)
    {
      gs_result = read.csv(paste("GAPIT.", method_store[i], ".Pred.result.csv", sep =
                                   ""),
                           head = T)
      m = nrow(gs_result)
    ...
    p <- plot_ly(
      ...
      text = ~ paste(
        ...
        taxa[pred_all$refinf == 1],
        "<br>Observed: ",
        round(observed[pred_all$refinf == 1], 4)  ,
        '<br>cBLUP:',
        round(cBLUP[pred_all$refinf == 1], 4)
      ),
      #size=2*y/max(y),
      color = I("blue"),
      symbol = I(1),
      ...
      text = ~ paste(
        ...
        round(observed[pred_all$refinf == 1], 4)  ,
        '<br>sBLUP:',
        round(sBLUP[pred_all$refinf == 1], 4)
      ),
      #size=2*y/max(y),
      color = I("green"),
      symbol = I(1),
      ...
      text = ~ paste(
        ...
        round(observed[pred_all$refinf > 1], 4)  ,
        '<br>gBLUP:',
        round(gBLUP[pred_all$refinf > 1], 4)
      ),
      #size=2*y/max(y),
      color = I("red"),
      symbol = I(4),
...
`GAPIT.Interactive.Manhattan` <-
  ...
  {
    if (is.null(GWAS))
      stop("Please add GWAS result in here!!!")
    ...
    plot.color = rep(c('#EC5f67',    '#FAC863',  '#99C794',    '#6699CC',  '#C594C5'),
                     ceiling(numCHR / 5))
    ...
    if (c("m") %in% plot.type)
    {
      ...
      p <- plot_ly(
        ...
        add_trace(
          y = bonferroniCutOff05,
          name = 'CutOff-0.05',
          color = I("red"),
          mode = "line",
          line = list(width = 1.4, dash = 'dot'),
          text = ""
        ) %>%
        layout(
      ...
      htmltools::save_html(p,
                           paste("Interactive.Manhattan.", name.of.trait, ".html", sep = ""))
    }
    ...
    if (c("q") %in% plot.type)
    {
      P.values = y
      p_value_quantiles <-
        (1:length(P.values)) / (length(P.values) + 1)
      log.P.values <- P.values
      log.Quantiles <- -log10(p_value_quantiles)
      ...
      htmltools::save_html(qp,
                           paste("Interactive.QQ ", name.of.trait, ".html", sep = ""))
...
#-----------------------------------------------------------------------
# Renamed rmx base fuction to avoid warnmings
#-----------------------------------------------------------------------
rmx <- function (...) {
...
`GAPIT.Liner` <-
  function(Y, GD, CV) {
    ...
    if (!is.null(CV))
      taxa = intersect(intersect(GD[, 1], Y[, 1]), CV[, 1])
    if (is.null(CV))
      taxa = intersect(GD[, 1], Y[, 1])
    
    Y = Y[match(taxa, Y[, 1], nomatch = 0), ]
    GD = GD[match(taxa, GD[, 1], nomatch = 0), ]
    
    if (!is.null(CV))
      CV = CV[match(taxa, CV[, 1], nomatch = 0), ]
    Y = Y[order(Y[, 1]), ]
    GD = GD[order(GD[, 1]), ]
    if (!is.null(CV))
      CV = CV[order(CV[, 1]), ]
...
`GAPIT.MAF` <-
  function(MAF = NULL,
    ...
    if (ncolors == -Inf)
    {
      ...
      pdf(paste("GAPIT.", trait, ".MAF.pdf" , sep = ""),
          width = 5,
          height = 5)
      par(mar = c(5, 6, 5, 3))
      ...
      plot(
        ...
        col = LPC,
        xlab = "MAF",
        ylab = expression(Probability ~  ~ -log[10](italic(p))),
        main = trait,
        cex.axis = 1.1,
        cex.lab = 1.3
      )
  ...
  function(Y,
    ...
    if (SUPER_GS)
    {
      ...
      if (!is.null(CV) & group.from < 1) {
        ...
        msgmsgmsg("The lower bound of groups should be 1 at least. It was set to 1!")
      }
      ...
      if (!is.null(KI))
      {
        if (group.to > nk) {
          ...
          msgmsgmsg("The upper bound of groups is too high. It was set to the size of kinship!")
        }
        if (group.from > nk) {
          group.from = nk
          msgmsgmsg("The lower bound of groups is too high. It was set to the size of kinship!")
        }
      }
...
`GAPIT.Manhattan` <-
  function(GI.MP = NULL,
    ...
    bin.mp = GI.MP[, 1:3]
    bin.mp[, 3] = 0 # for r2
    bin.mp[, 1] = as.numeric(as.vector(GI.MP[, 2])) + as.numeric(as.vector(GI.MP[, 1])) *
      (10 ^ (max(GI.MP[, 1]) + 1))
    ...
    if (plot.type == "Chromosomewise" & !is.null(GD))
    {
      ...
      pdf(
        paste(
          "GAPIT.",
          name.of.trait,
          ".Manhattan.Plot.Chromosomewise.pdf" ,
          sep = ""
        ),
        width = 10
      )
      ...
      for (i in 1:numCHR)
      {
        ...
        # print(head(subset))
        if (nrow(subset) == 0)
          next #thanks to lvclark to fix it
        ...
        num.row = nrow(sig.mp)
        if (!is.null(dim(sig.mp)))
          sig.mp = sig.mp[!duplicated(sig.mp[, 7]), ]
        num.row = nrow(sig.mp)
        if (is.null(dim(sig.mp)))
          num.row = 1
        bin.set = NULL
        ...
        print(
          paste(
            "select ",
            num.row,
            " candidate significont markers in ",
            i,
            " chromosome ",
            sep = ""
          )
        ...
        if (length(unique(sig.index)) == 2)
        {
          for (j in 1:num.row)
          {
            ...
            #print(dim(bin.store))
            if (is.null(CG))
              candidata = bin.store[bin.store[, 3] == max(bin.store[, 3]), 5]
        ...
        #print(dim(X))
        y.lim <-
          ceiling(max(subset[, 3])) + 1  #set upper for each chr
        if (length(subset) > 3) {
        ...
        r2_color[which(r2_color[, 2] <= 0.4 &
                         r2_color[, 2] > 0.2), 2] = do_color[n_col * 0.8]
        r2_color[which(r2_color[, 2] <= 0.6 &
                         r2_color[, 2] > 0.4), 2] = do_color[n_col * 0.6]
        r2_color[which(r2_color[, 2] <= 0.8 &
                         r2_color[, 2] > 0.6), 2] = do_color[n_col * 0.4]
        r2_color[which(r2_color[, 2] <= 1 &
                         r2_color[, 2] > 0.8), 2] = do_color[n_col / n_col]
        ...
        plot(
          ...
          xlim = c(min(x), max(x)),
          col = r2_color[, 2],
          xlab = expression(Base ~ Pairs ~ (x10 ^ -6)),
          ylab = "-Log Base 10 p-value",
          main = 			paste("Chromosome", chm.to.analyze[i], sep = " "),
          cex.lab = 1.6,
          pch = 21,
          bg = r2_color[, 2]
        )
        ...
        barplot(
          matrix(rep(1, times = n_col), n_col, 1),
          beside = T,
          col = do_color,
          border = do_color,
          axes = FALSE,
    ...
    if (plot.type == "Genomewise")
    {
      ...
      thecolor = cycle1
      for (i in 2:ncycle) {
        thecolor = c(thecolor, cycle1 + (i - 1))
      }
      ...
      col.Rushville = rep(c("orangered", "navyblue"), ceiling(numCHR /
                                                                2))
      col.Congress = rep(c("deepskyblue3", "firebrick"), ceiling(numCHR /
                                                                   2))
      ...
      col.Oceanic = rep(c('#EC5f67',		'#FAC863',	'#99C794',		'#6699CC',	'#C594C5'),
                        ceiling(numCHR / 5))
      col.cougars = rep(c('#990000',		'dimgray'), ceiling(numCHR / 2))
      
      if (plot.style == "Rainbow")
        plot.color = col.Rainbow
      if (plot.style == "FarmCPU")
        plot.color = col.Rainbow
      if (plot.style == "Rushville")
        plot.color = col.Rushville
      if (plot.style == "Congress")
        plot.color = col.Congress
      if (plot.style == "Ocean")
        plot.color = col.Ocean
      if (plot.style == "PLINK")
        plot.color = col.PLINK
      if (plot.style == "Beach")
        plot.color = col.Beach
      if (plot.style == "Oceanic")
        plot.color = col.Oceanic
      if (plot.style == "cougars")
        plot.color = col.cougars
      ...
      #print(chor_taxa)
      if (length(chor_taxa) != length(ticks))
        chor_taxa = NULL
...
`GAPIT.Multiple.Manhattan` <-
  function(model_store,
    ...
    for (i in 1:length(model_store))
    {
      for (j in 1:(ncol(Y) - 1))
      {
        environ_name = c(environ_name,
                         paste(model_store[i], ".", colnames(Y)[-1][j], sep = ""))
      }
    }
    sig_pos = NULL
    simulation = FALSE
    ...
    for (i in 1:length(environ_name))
    {
      ...
      environ_result = read.csv(paste("GAPIT.", environ_name[i], ".GWAS.Results.csv", sep =
                                        ""),
                                head = T)
      ...
      write.table(y_filter,
                  paste("Filter_", environ_name[i], "_GWAS_result.txt", sep = ""))
      ...
      if (i != 1) {
        result0 = merge(result0,
                        result[, c(1, 4)],
                        by.x = colnames(result0)[1],
                        by.y = colnames(result)[1])
        colnames(result0)[i + 3] = environ_name[i]
      }
      rownames(result) = 1:nrow(result)
      result[is.na(result[, 4]), 4] = 1
      sig_pos = append(sig_pos, as.numeric(rownames(result[result[!is.na(result[, 4]), 4] <
                                                             (cutOff / nrow(result)), ])))
    ...
    pdf(
      paste("GAPIT.Manhattan.Mutiple.Plot.high", ".pdf" , sep = ""),
      width = 20,
      height = 6 * Nenviron
    )
    ...
    for (k in 1:Nenviron)
    {
      ...
      environ_result = read.csv(paste("GAPIT.", environ_name[k], ".GWAS.Results.csv", sep =
                                        ""),
                                head = T)
    ...
    # print(result0)
    pdf(
      paste("GAPIT.Manhattan.Mutiple.Plot.wide", ".pdf" , sep = ""),
      width = 16,
      height = 8.5
    )
    par(mfrow = c(Nenviron, 1))
...
`GAPIT.Numericalization` <-
  function(x,
    ...
    if (Major.allele.zero) {
      if (len > 1 & len <= 3) {
        ...
        if (bit == 1) {
          ...
          count.temp <-
            count.temp[order(count.temp[, 1], decreasing = TRUE), ]
          if (len == 3)
        ...
        if (bit == 2) {
          ...
          count.temp <-
            count.temp[order(count.temp[, 1], decreasing = TRUE), ]
          if (len == 3)
            order =  c(count.temp[1, 2], 2, count.temp[2, 2])
          else
            order = count.temp[, 2]
        }
    ...
    if (bit == 1) {
      if (len == 3)
        x = ifelse(x == "N", NA, ifelse(x == lev[1], 0, ifelse(x == lev[3], 1, 2)))
    } else{
      if (len == 3)
        x = ifelse(x == "N", NA, ifelse(x == lev[lev != inter][1], 0, ifelse(x ==
                                                                               inter, 1, 2)))
    }
    ...
    if (file.output & PC.number > 1) {
      ...
      if (PCA.3d == TRUE)
      {
        ...
        plot3d(
          ...
          min(PCA2),
          min(PCA3),
          xlim = c(min(PCA1), max(PCA1)),
          ylim = c(min(PCA2), max(PCA2)),
          zlim = c(min(PCA3), max(PCA3)),
          xlab = "PCA1",
          ylab = "PCA2",
          zlab = "PCA3",
        ...
        if (num_col == 1)
        {
          ...
          widgets <-
            rglwidget(width = 900, height = 900) %>% toggleWidget(ids = sids1, label = "PCA")
        } else if (num_col == 2)
        {
          ...
          sids1 <-
            spheres3d(PCA1[index1],
                      PCA2[index1],
                      PCA3[index1],
                      col = PCA.col[index1],
                      radius = 1)
          sids2 <-
            spheres3d(PCA1[index2],
                      PCA2[index2],
                      PCA3[index2],
                      col = PCA.col[index2],
                      radius = 1)
          widgets <-
            rglwidget(width = 900, height = 900) %>% toggleWidget(ids = sids1, label = "Population 1") %>%
            toggleWidget(ids = sids2, label = "Population 2")
        } else if (num_col == 3)
        {
          index1 = PCA.col == unique(PCA.col)[1]
          index2 = PCA.col == unique(PCA.col)[2]
          ...
          widgets <-
            rglwidget(width = 900, height = 900) %>% toggleWidget(ids = sids1, label = "Population 1") %>%
            toggleWidget(ids = sids2, label = "Population 2") %>% toggleWidget(ids = sids3, label = "Population 3")
        } else if (num_col == 4)
        {
          index1 = PCA.col == unique(PCA.col)[1]
          index2 = PCA.col == unique(PCA.col)[2]
          ...
          widgets <-
            rglwidget(width = 900, height = 900) %>% toggleWidget(ids = sids1, label = "Population 1") %>%
            toggleWidget(ids = sids2, label = "Population 2") %>% toggleWidget(ids = sids3, label = "Population 3") %>%
            toggleWidget(ids = sids4, label = "Population 4")
        }
        if (interactive())
          widgets
...
`GAPIT.PCA2Power` <-
  function(myGD = NULL,
    ...
    if (is.null(myGD) ||
        is.null(myGM)) {
      stop("Read data Invalid. Please select read valid flies !")
    }
    ...
    plot(
      power_npca[, 1] ~ fdr_npca[, 1],
      bg = "lightgray",
      xlab = "FDR",
      ylab = "Power",
      ylim = c(0, 1),
      xlim = c(0, 1),
      main = "Power against FDR",
      type = "o",
      pch = 20,
      col = ncol[1],
      cex = 1,
...
`GAPIT.Phenotype.PCA.View` <- function(PC = NULL, myY = NULL) {
  ...
  if (is.null(PC)) {
    stop("Validation Invalid. Please input four PC value  !")
  }
  if (is.null(myY)) {
    stop("Validation Invalid. Please select read valid Phenotype flies  !")
  }
  ...
  y.mean = mean(y2[, 2])
  n.col = 54
  y.int = round(abs(y2[, 2] - y.mean) / y.range * (.5 * n.col - 1) * 2) +
    1
  mycol = rainbow(n.col)
  y.col = mycol[y.int]
  ...
  pdf(
    paste("GAPIT.", traitname, "_vs_PC.pdf", sep = ""),
    width = 9,
    height = 6
  )
  #par(mar = c(5,5,5,5))
...
`GAPIT.Phenotype.View` <- function(myY = NULL,
  ...
  pdf(
    paste("GAPIT", memo, traitname, "phenotype_view.pdf", sep = "."),
    width = 10,
    height = 6
  )
  par(mar = c(5, 5, 5, 5))
...
`GAPIT.Power.Compare` <-
  function(GD = NULL,
    ...
    if (is.null(Method))
      stop("Please choose methods to compare !!")
    MM = length(Method)
    X <- GD[, -1]
    ...
    rep.power.GLM <- data.frame(matrix(0, maxOut, MM))
    rep.FDR.GLM <- data.frame(matrix(0, maxOut, MM))
    rep.Power.Alpha.GLM <- data.frame(matrix(0, 12, MM))
    
    ##PCA
    ##---------------------##
    if (!is.null(PCA.total)) {
      PCA <- prcomp(X)
      PCVar <- PCA$sdev ^ 2
      myPC <- PCA$x[, 1:PCA.total]
    ...
    if (is.null(CV)) {
      ...
      colnames(myPC)[1] = colnames(CV) = c("Taxa")
      myCV = merge(CV, myPC, by.x = "Taxa", by.y = "Taxa")
    }
    ...
    for (i in 1:rep)
    {
      print("Start to simulate Phenotype with genotype !!")
      mysimulation <-
        GAPIT.Phenotype.Simulation(
          GD = GD,
          GM = GM,
      ...
      posi = mysimulation$QTN.position
      myY = mysimulation$Y
      print("Done to simulate Phenotype with genotype !!")
      for (j in 1:MM)
      {
        print(
          paste(
        ...
        rep.power.GLM[, j] <- rep.power.GLM[, j] + power$Power
        rep.FDR.GLM[, j] <- rep.FDR.GLM[, j] + power$FDR
        rep.Power.Alpha.GLM[, j] <-
          rep.Power.Alpha.GLM[, j] + power$Power.Alpha
      }
    ...
    write.csv(rep.power.GLM,
              paste(h2, "_", NQTN, ".Power.GLM", rep, ".csv", sep = ""))
    write.csv(rep.FDR.GLM,
              paste(h2, "_", NQTN, ".FDR.GLM", rep, ".csv", sep = ""))
    write.csv(rep.Power.Alpha.GLM,
              paste(h2, "_", NQTN, ".Alpha.GLM", rep, ".csv", sep = ""))
    ...
    for (i in 1:MM)
    {
      mMethod = paste(mMethod, Method[i], ".", sep = "")
    }
    
    pdf(
      paste(
        "GAPIT.Power.Compare with ",
        mMethod,
        " in Simulated",
        ".pdf",
        sep = ""
    ...
    plot(
      ...
      lwd = 2,
      las = 1,
      ylim = c(min(rep.Power.Alpha.GLM), max(rep.Power.Alpha.GLM))
    )
    ...
    if (MM != 1)
    {
      ...
      {
        lines(
          rep.Power.Alpha.GLM[, i] ~ myalpha1,
          lwd = 2,
          type = "o",
          pch = 20,
          col = i
    ...
    print(paste("~~ GAPIT.Power.Compare Done Successfully! ~~" , sep = ""))
  ...
  function(myG = null,
    ...
    for (i in 1:rel)
    {
      ...
      myGAPIT_SUPER <- GAPIT(
        ...
        #PCA.total=3,
        sangwich.top = "MLM",
        #options are GLM,MLM,CMLM, FaST and SUPER
        sangwich.bottom = "SUPER",
        #options are GLM,MLM,CMLM, FaST and SUPER
        LD = 0.1,
        QTN.position = QTN.position,
      ...
      system(paste(
        '"plink.exe"',
        paste(
          '--tped mdp_numeric.tped --tfam mdp_numeric',
          i,
          '.tfam --assoc --out gwas.mdp_numeric',
          i,
          sep = ''
        )
      ), wait = TRUE)
      ...
      system(paste(
        ...
        paste(
          '--tped mdp_numeric.tped --tfam mdp_numeric',
          i,
          '.tfam --covar pca.txt --linear --hide-covar  --out gwas.mdp_numeric',
          i,
          sep = ''
        )
      ), wait = TRUE)
      ...
      plinkGWAS <-
        read.table(paste("gwas.mdp_numeric", i, ".assoc.linear", sep = ""),
                   header = T)
      
      Format_GWAS = cbind(myGM,
                          plinkGWAS[, 9],
                          rep(NA, nrow(myGM)),
                          rep(NA, nrow(myGM)),
                          rep(NA, nrow(myGM)))
      ...
      rep.Power.Alpha.SUPER <-
        rep.Power.Alpha.SUPER + myGAPIT_SUPER$Power.Alpha
    ...
    write.csv(
      cbind(rep.FDR.GLM, rep.power.GLM),
      paste(h2, "_", NQTN, ".Power.by.FDR.GLM", rel, ".csv", sep = "")
    )
    write.csv(
      cbind(myalpha, rep.Power.Alpha.GLM),
      paste(h2, "_", NQTN, ".Power.by.TypeI.GLM", ".csv", sep = "")
    )
    
    write.csv(
      cbind(rep.FDR.MLM, rep.power.MLM),
      paste(h2, "_", NQTN, ".Power.by.FDR.MLM", rel, ".csv", sep = "")
    )
    write.csv(
      cbind(myalpha, rep.Power.Alpha.MLM),
      paste(h2, "_", NQTN, ".Power.by.TypeI.MLM", ".csv", sep = "")
    )
    
    write.csv(
      cbind(rep.FDR.SUPER, rep.power.SUPER),
      paste(h2, "_", NQTN, ".Power.by.FDR.SUPER", rel, ".csv", sep = "")
    )
    write.csv(
      cbind(myalpha, rep.Power.Alpha.SUPER),
      paste(h2, "_", NQTN, ".Power.by.TypeI.SUPER", ".csv", sep = "")
    )
    
    write.csv(
      cbind(rep.FDR.CMLM, rep.power.CMLM),
      paste(h2, "_", NQTN, ".Power.by.FDR.CMLM", rel, ".csv", sep = "")
    )
    write.csv(
      cbind(myalpha, rep.Power.Alpha.CMLM),
      paste(h2, "_", NQTN, ".Power.by.TypeI.CMLM", ".csv", sep = "")
    )
    
    write.csv(
      cbind(rep.FDR.ECMLM, rep.power.ECMLM),
      paste(h2, "_", NQTN, ".Power.by.FDR.ECMLM", rel, ".csv", sep = "")
    )
    write.csv(
      cbind(myalpha, rep.Power.Alpha.ECMLM),
      paste(h2, "_", NQTN, ".Power.by.TypeI.ECMLM", ".csv", sep = "")
    )
    
    write.csv(
      cbind(rep.FDR.plink, rep.power.plink),
      paste(h2, "_", NQTN, ".Power.by.FDR.plink", rel, ".csv", sep = "")
    )
    write.csv(
      cbind(myalpha, rep.Power.Alpha.plink),
      paste(h2, "_", NQTN, ".Power.by.TypeI.plink", ".csv", sep = "")
    )
    ...
    write.csv(
      ...
      paste(
        ...
        "_",
        NQTN,
        ".Power.by.FDR.GLM.MLM.SUPER.plink",
        rel,
        ".csv",
        sep = ""
      )
    ...
    pdf(
      paste(
        "GAPIT.Power ",
        name.of.trait,
        ".compare to GLM,MLM,CMLM,ECMLM,SUPER.plink",
        ".pdf",
        sep = ""
      ),
      width = 4.5,
    ...
    lines(
      rep.power.ECMLM[, 6] ~ rep.FDR.ECMLM[, 6],
      lwd = 2,
      type = "o",
      pch = 20,
      col = 2
    ...
    lines(
      rep.power.CMLM[, 6] ~ rep.FDR.CMLM[, 6],
      lwd = 2,
      type = "o",
      pch = 20,
      col = 3
    ...
    lines(
      rep.power.plink[, 6] ~ rep.FDR.plink[, 6],
      lwd = 2,
      type = "o",
      pch = 20,
      col = 6,
    ...
    legend(
      "bottomright",
      c("SUPER", "ECMLM", "CMLM", "MLM", "GLM", "PLINK"),
    ...
    kkt <-
      cbind(
        rep.Power.Alpha.SUPER[, 1],
        rep.Power.Alpha.ECMLM[, 1],
        rep.Power.Alpha.CMLM[, 1],
        rep.Power.Alpha.MLM[, 1],
        rep.Power.Alpha.GLM[, 1],
        rep.Power.Alpha.plink[, 1]
      )
    write.csv(
      cbind(
        myalpha,
      ...
      paste(
        ...
        "_",
        NQTN,
        ".Type I error.Power.by.FDR.GLM.MLM.SUPER",
        rel,
        ".csv",
        sep = ""
      )
    ...
    plot(
      ...
      lwd = 2,
      las = 1,
      ylim = c(min(kkt), max(kkt))
    )
    lines(
      rep.Power.Alpha.ECMLM[, 1] ~ myalpha1,
      lwd = 2,
      type = "o",
      pch = 20,
      col = 2
    ...
    lines(
      rep.Power.Alpha.CMLM[, 1] ~ myalpha1,
      lwd = 2,
      type = "o",
      pch = 20,
      col = 3
    ...
    lines(
      rep.Power.Alpha.MLM[, 1] ~ myalpha1,
      lwd = 2,
      type = "o",
      pch = 20,
      col = 4
    ...
    lines(
      rep.Power.Alpha.GLM[, 1] ~ myalpha1,
      lwd = 2,
      type = "o",
      pch = 20,
      col = 5
    ...
    lines(
      rep.Power.Alpha.plink[, 1] ~ myalpha1,
      lwd = 2,
      type = "o",
      pch = 20,
      col = 6,
...
`GAPIT.Prediction` <- function(myK = NULL,
                               y = NULL,
                               num = NULL) {
  ...
  if (is.null(myK) ||
      is.null(y)) {
    stop("Validation Invalid. Please select read valid flies !")
  }
  if (is.null(num))
  {
  ...
  if (num < 2 ||
      num > m) {
    stop("Validation Invalid. Please select folders num >1 !")
  }
  ...
  for (j in 2:num)
  {
    ...
    {
      vali[, j] <-
        unique(as.character(sample(y[!(y$Taxa %in% vali[, 1:j - 1]), 'Taxa'], m.sample)))
    }
    if (j == num)
    {
    ...
    if (j != num)
      cali[, j] <-
        unique(as.character(y[!(y$Taxa %in% vali[, j]), 'Taxa']))
    if (j == num)
  ...
  commonGeno_v <-
    lines.vali[lines.vali %in% myK[, 1]]
  ...
  blup_prediction.ref <-
    blup_prediction[match(commonGeno_c, blup_prediction$Taxa), ]
  blup_prediction.inf <-
    blup_prediction[match(commonGeno_v, blup_prediction$Taxa), ]
  ...
  legend(
    "bottomright",
    paste("R^2=", format(kr$coefficients[2], digits = 4), seq = ""),
    col = "white",
    text.col = "blue",
    lwd = 2,
    cex = 1.2,
  ...
  legend(
    "bottomright",
    paste("R^2=", format(ki$coefficients[2], digits = 4), seq = ""),
    col = "white",
    text.col = "blue",
    lwd = 2,
    cex = 1.2,
...
`GAPIT.QC2` <-
  function(Y = NULL,
    ...
    if (!is.null(Z) & !is.null(KI))
    {
      ...
      # print(taxa.kinship)
      # print(taxa.Z)
      if (!identical(taxa.kinship, taxa.Z))
        Z <- cbind(Z[, 1], Z[, 1 + match(taxa.Z_K_common, taxa.Z, nomatch = 0)])
      ...
      Z5 = as.matrix(Z1)
      RS = apply(Z5, 1, function(one)
        sum(as.numeric(one))) > 0
...
`GAPIT.QQ` <-
  function(P.values,
    ...
    if (length(P.values[P.values > 0]) < 1)
      return(NULL)
    N = length(P.values)
    ...
    if (plot.type == "log_P_values")
    {
      ...
      if (plot.style == "FarmCPU") {
        pdf(
          paste("FarmCPU.", name.of.trait, ".QQ-Plot.pdf" , sep = ""),
          width = 5,
          height = 5
        )
        par(mar = c(5, 6, 5, 3))
      ...
      if (plot.style == "rainbow") {
        pdf(
          paste("GAPIT.", name.of.trait, ".QQ-Plot.pdf" , sep = ""),
          width = 5,
          height = 5
        )
        par(mar = c(5, 6, 5, 3))
      ...
      plot(
        NULL,
        xlim = c(0, max(log.Quantiles)),
        ylim = c(0, max(log.P.values)),
        type = "l",
        lty = 5,
        lwd = 2,
        axes = FALSE,
    ...
    if (plot.type == "P_values")
    {
      ...
      qqplot(
        ...
        ylim = c(0, 1),
        type = "l" ,
        xlab = "Uniform[0,1] Theoretical Quantiles",
        lty = 1,
        lwd = 1,
        ylab = "Quantiles of P-values from GWAS",
        col = "Blue",
        main = paste(name.of.trait, sep = " ")
      )
...
`GAPIT.ROC` <-
  function(t = NULL,
    ...
    if (plot.style == "FarmCPU") {
      write.table(
        power,
        file = paste("FarmCPU.", trait, ".ROC.csv", sep = ""),
        quote = TRUE,
        sep = ",",
        row.names = TRUE,
        col.names = NA
    ...
    if (plot.style == "rainbow") {
      write.table(
        power,
        file = paste("GAPIT.", trait, ".ROC.csv", sep = ""),
        quote = TRUE,
        sep = ",",
        row.names = TRUE,
        col.names = NA
    ...
    if (plot.style == "FarmCPU") {
      pdf(
        paste("FarmCPU.", trait, ".ROC.pdf" , sep = ""),
        width = 5,
        height = 5
      )
      par(mar = c(5, 6, 5, 3))
    ...
    if (plot.style == "rainbow") {
      pdf(paste("GAPIT.", trait, ".ROC.pdf" , sep = ""),
          width = 7,
          height = 7)
      par(mar = c(5, 5, 5, 3))
    }
...
`GAPIT.RandomModel` <-
  function(GWAS,
    ...
    if (in_True == 1)
      colnames(tree2)[ncol(tree2)] = paste("gene_", 1, sep = "")
    ...
    write.csv(
      ...
      paste(
        "GAPIT.",
        name.of.trait,
        ".Phenotype_Variance_Explained_by_Association_Markers.csv",
        sep = ""
      ),
      quote = FALSE,
      sep = ",",
    ...
    {
      pdf(
        paste("GAPIT.", name.of.trait, ".Effect_VP.pdf" , sep = ""),
        width = 7,
        height = 5.75
      )
      par(mar = c(4, 5, 4, 4))
      ...
      pdf(
        paste("GAPIT.", name.of.trait, ".MAF_VP.pdf" , sep = ""),
        width = 7,
        height = 5.75
      )
      par(mar = c(4, 5, 4, 4))
      ...
      {
        pdf(
          paste("GAPIT.", name.of.trait, ".MAF_Effect_VP.pdf" , sep = ""),
          width = 9,
          height = 5.75
        )
        ...
        for (i in 1:(n / 2))
        {
          r2_color[z <= quantile_cut[i + 1] & z >= quantile_cut[i]] = do_color[2 *
                                                                                 i]
        }
        plot(
          y ~ x,
        ...
        small_bar = round(seq(0, round(max(z), 3), by = (max(z) - min(z)) /
                                10), 2)
        #main()
        mtext(
        ...
        axis(
          4,
          c(1, 6, 11),
          c(min(small_bar), median(small_bar), max(small_bar)),
          las = 2,
          cex.axis = 0.7,
          tick = F,
          line = 0
...
`GAPIT.Report` <-
  function(name.of.trait = NULL,
    ...
    index = 1:nrow(GWAS)
    if (threshold.output < 1)
      index = which(GWAS[, 4] < threshold.output)
    if (plot.style == "FarmCPU") {
      write.table(
        GWAS[index, ],
        paste("FarmCPU.", name.of.trait, ".GWAS.Results.csv", sep = ""),
        quote = FALSE,
        sep = ",",
        row.names = FALSE,
        col.names = TRUE
...
`GAPIT.SUPER.FastMLM` <-
  function(ys,
    ...
    g <- nrow(ys)
    n <-
      nrow(xs)   #####  generaol nrow(xs)=nrow(U1) rivised by qishan 2012.4.16
    m <- ncol(xs)
    t <- nrow(xs)
    q0 <- ncol(X0)
    q1 <- q0 + 1
      ...
      for (j in 1:g)
      {
        for (i in 1:m)
        {
          ...
          if (min(xs[, i]) != max(xs[, i]))
          {
            ...
            U <-
              U1 * matrix(sqrt(1 / (d + delta)), nrow(U1), length(d), byrow = TRUE)
            Xt <- crossprod(U, X)
            XX1 <- crossprod(Xt, Xt)
            XX2 <-
              crossprod((I - tcrossprod(U1, U1)) %*% X, (I - tcrossprod(U1, U1)) %*% X) /
              delta
            #iXX<-solve(XX1+XX2)
  ...
  function(Y,
    ...
    if (super_pass)
    {
      ...
      if (is.null(GK)) {
        ...
        #print(snpsam)
        if (snpsam == 1)
          stop ("GAPIT says: SUPER_GS must putin GD and GM.")
        GK = GK[, SNPVar > 0]
        GK = cbind(as.data.frame(GT[GTindex]), as.data.frame(GK)) #add taxa
      ...
      if (inclosure.to > nrow(Y))
        ##########removed by Jiabo Wang ,unlimited number of inclosures
      {
        inclosure.to = nrow(Y) - 1
      ...
      if (inclosure.from > inclosure.to)
        ##########removed by Jiabo Wang ,unlimited number of inclosures
      {
      ...
      if (is.null(dim(SUPER_selection)))
      {
        ...
        optimum_SUPER = SUPER_selection[which(as.numeric(SUPER_selection[, 4]) ==
                                                min(as.numeric(SUPER_selection[, 4]))), ]
      }
      print(optimum_SUPER)
      ########################BUILD SUPER KINSHIP
      ##########################################################
      ...
      if (kinship.algorithm == "Loiselle")
        SUPER_myKI_test = GAPIT.kinship.loiselle(snps = t(as.matrix(.5 * (
          SUPER_optimum_GD[, -1]
        ))),
        method = "additive",
        use = "all")
      ...
      if (kinship.algorithm == "Zhang")
        SUPER_myKI_test = GAPIT.kinship.Zhang(snps = as.matrix(SUPER_optimum_GD[, -1]))
      if (!kinship.algorithm == "Loiselle" |
          !kinship.algorithm == "Zhang")
        SUPER_myKI_test = GAPIT.kinship.VanRaden(snps = as.matrix(SUPER_optimum_GD[, -1]))
      ...
      for (ca in kinship.cluster)
      {
        for (kt in kinship.group)
        {
          ...
          if (order_count == 1)
            print("-------Mixed model with Kinship-----------------------------")
          if (group < ncol(X0) + 1)
            group = 2 # the emma function (emma.delta.REML.dLL.w.Z) does not allow K has dim less then CV. turn to GLM (group=1)
          # print(Sys.time())
          if (nrow(KI) == group)
          {
          ...
          Z = matrix(as.numeric(as.matrix(zc$Z[, -1])),
                     nrow = zrow,
                     ncol = zcol)
          if (is.null(dim(ys)) ||
              ncol(ys) == 1)
          ...
          {
            x.inv <-
              try(tcrossprod(X %*% solve(crossprod(X)), X), silent = TRUE)
          ...
          print(
            paste(
              ...
              "--",
              "Vg=",
              round(emma_test$Vu, 4),
              "VE=",
              round(emma_test$Ve, 4),
              "-2LL=",
              round(-2 * emma_test$loglik, 2),
              "  Clustering=",
              ca,
              "  Group number=",
              group ,
    ...
    Compression = Compression[order(as.numeric(Compression[, 4]), decreasing = FALSE), ]
    Compression = matrix(Compression, ncol = 6, byrow = T)
    print(Compression)
    ...
    if (nrow(Compression) > 1)
    {
      cp <-
        GAPIT.Compress(
          KI = KI,
          kinship.cluster = optimum_Clustering,
          kinship.group = optimum_groupK,
          GN = optimum_group,
          Timmer = Timmer,
          Memory = Memory
        )
    ...
    gs <-
      GAPIT.GS(
        ...
        KWO = bk$KWO,
        GAU = bk$GAU,
        UW = cbind(emma_REMLE$uhat, emma_REMLE$PEVuhat)
      )
    BB = merge(gs$BLUP,
               emma_BLUE,
               by.x = "Taxa",
    ...
    if (file.output)
      write.csv(
        all_gs,
        paste("GAPIT.", model, ".Pred.result.csv", sep = ""),
        row.names = FALSE,
        col.names = TRUE
      )
    ...
    return (
      list(
        ...
        Compression = Compression,
        kinship = my_allKI,
        SUPER_kinship = SUPER_myKI,
        SUPER_GD = SUPER_optimum_GD ,
        PC = my_allCV,
        Timmer = Timmer,
        Memory = Memory,
        GWAS = NULL,
...
`GAPIT.Specify` <-
  function(GI = NULL,
    ...
    ID.GP = as.numeric(as.vector(GP[, 3])) + as.numeric(as.vector(GP[, 2])) *
      MaxBP
    ...
    theIndex = NULL
    if (!is.null(GI)) {
      ID.GI = as.numeric(as.vector(GI[, 3])) + as.numeric(as.vector(GI[, 2])) *
        MaxBP
      #print("ID.GI")
      #print(ID.GI)
...
`GAPIT.Table` <-
  function(final.table = final.table,
           name.of.trait = name.of.trait,
           SNP.FDR = 1) {
...
`GAPIT.cross_validation.compare` <-
  function(myGD = NULL,
           y = NULL,
           rel = NULL,
           tc = NULL) {
    ...
    if (rel < 2) {
      stop("Validation Invalid. Please select replications >1 !")
    }
    #rel<-2 ##replications
    #t<-2
    y <- y[!is.na(y[, 2]), ]
    ...
    #############
    commonGeno <-
      unique(as.character(y[, 1]))[unique(as.character(y[, 1])) %in% myGD[, 1]]
    ...
    for (w in 1:nrow(tc1)) {
      ...
      for (k in 1:rel)
      {
        ...
        for (i in 1:num) {
          ...
          if (r.inf < 0) {
            ...
            write.csv(combine_output,
                      paste("Accuracy_folders", num, k, i, rel, ".csv", sep = ""))
            #stop("...........")
          }
      ...
      write.csv(
        combine_output,
        paste("Accuracy_folders", num, "by CMLM,rel_", rel, ".csv", sep = "")
      )
      write.csv(
        combine_output1,
        paste(
          "Accuracy_folders  ref",
          num,
          "by CMLM,rel_",
          rel,
          ".csv",
    ...
    pdf(
      paste(
        "GAPIT.cross_validation ",
        name.of.trait,
        sj,
        ".compare to different folders.",
        ".pdf",
        sep = ""
      ),
      width = 4.5,
...
emma.kinship <- function(snps, method = "additive", use = "all") {
  ...
  if (method == "dominant") {
    flags <-
      matrix(as.double(rowMeans(snps, na.rm = TRUE) > 0.5), nrow(snps), ncol(snps))
    snps[!is.na(snps) &&
           (snps == 0.5)] <- flags[!is.na(snps) && (snps == 0.5)]
  }
...
emma.eigen.R.wo.Z <- function(K, X) {
  n <- nrow(X)
  q <- ncol(X)
  S = try(diag(n) - X %*% solve(crossprod(X, X)) %*% t(X), silent = TRUE)
  if (inherits(S, "try-error"))
  {
    S <- diag(n) - X %*% MASS::ginv(crossprod(X, X)) %*% t(X)
  }
  eig <- eigen(S %*% (K + diag(1, n)) %*% S, symmetric = TRUE)
  stopifnot(!is.complex(eig$values))
  return(list(values = eig$values[1:(n - q)] - 1, vectors = eig$vectors[, 1:(n -
                                                                               q)]))
}
...
emma.eigen.R.w.Z <- function(Z, K, X, complete = TRUE) {
  ...
  return(list(
    ...
    vectors = qr.Q(qr(cbind(
      SZ %*% eig$vectors[, 1:(t - q)], qr.X
    )),
...
emma.delta.ML.LL.wo.Z <- function(logdelta, lambda, etas, xi) {
  ...
  return(0.5 * (n * (log(n / (
    2 * pi
  )) - 1 - log(sum((etas * etas) / (lambda + delta)
  ))) - sum(log(xi + delta))))
}
...
emma.delta.ML.LL.w.Z <-
  function(logdelta,
    ...
    return(0.5 * (n * (log(n / (
      ...
      sum(etas.1 * etas.1 / (lambda + delta)) + etas.2.sq / delta
    )) - (sum(log(
      xi.1 + delta
    )) + (n - t) * logdelta)))
  }
...
emma.delta.ML.dLL.w.Z <-
  function(logdelta,
    ...
    ldelta <- lambda + delta
    return(0.5 * (
      n * (sum(etasq / (ldelta * ldelta)) + etas.2.sq / (delta * delta)) / (sum(etasq /
                                                                                  ldelta) + etas.2.sq / delta) - (sum(1 / (xi.1 + delta)) + (n - t) / delta)
    ))
  }
...
emma.delta.REML.LL.wo.Z <- function(logdelta, lambda, etas) {
  nq <- length(etas)
  delta <-  exp(logdelta)
  return(0.5 * (nq * (log(nq / (
    2 * pi
  )) - 1 - log(sum(
    etas * etas / (lambda + delta)
...
emma.delta.REML.LL.w.Z <-
  function(logdelta, lambda, etas.1, n, t, etas.2.sq) {
    ...
    delta <-  exp(logdelta)
    return(0.5 * (nq * (
      log(nq / (2 * pi)) - 1 - log(sum(etas.1 * etas.1 / (lambda + delta)) + etas.2.sq /
                                     delta)
    ) - (sum(
      log(lambda + delta)
    ) + (n - t) * logdelta)))
...
emma.delta.REML.dLL.w.Z <-
  function(logdelta, lambda, etas.1, n, t1, etas.2.sq) {
    ...
    ldelta <- lambda + delta
    return(0.5 * (
      nq * (sum(etasq / (ldelta * ldelta)) + etas.2.sq / (delta * delta)) / (sum(etasq /
                                                                                   ldelta) + etas.2.sq / delta) - (sum(1 / ldelta) + (n - t) / delta)
    ))
  }
                     ...
                     eig.R = NULL)
{
  ...
  if (is.null(Z)) {
    ...
    delta <- exp(logdelta)
    Lambdas <-
      matrix(eig.R$values, n - q, m) + matrix(delta, n - q, m, byrow = TRUE)
    Xis <- matrix(eig.L$values, n, m) + matrix(delta, n, m, byrow = TRUE)
    Etasq <- matrix(etas * etas, n - q, m)
    LL <-
      0.5 * (n * (log(n / (2 * pi)) - 1 - log(colSums(Etasq / Lambdas))) - colSums(log(Xis)))
    dLL <-
      0.5 * delta * (n * colSums(Etasq / (Lambdas * Lambdas)) / colSums(Etasq /
                                                                          Lambdas) - colSums(1 / Xis))
    ...
    if (dLL[1] < esp) {
      ...
      optLL <-
        append(optLL,
               emma.delta.ML.LL.wo.Z(llim, eig.R$values, etas, eig.L$values))
    }
    if (dLL[m - 1] > 0 - esp) {
      optlogdelta <- append(optlogdelta, ulim)
      optLL <-
        append(optLL,
               emma.delta.ML.LL.wo.Z(ulim, eig.R$values, etas, eig.L$values))
    }
    ...
    for (i in 1:(m - 1))
    {
          ...
          (dLL[i] > 0) && (dLL[i + 1] < 0))
      {
        ...
        optLL <-
          append(optLL,
                 emma.delta.ML.LL.wo.Z(r$root, eig.R$values, etas, eig.L$values))
      }
    }
    ...
    delta <- exp(logdelta)
    Lambdas <-
      matrix(eig.R$values, t - q, m) + matrix(delta, t - q, m, byrow = TRUE)
    ...
    dLL <-
      0.5 * delta * (n * (colSums(Etasq / (
        Lambdas * Lambdas
      )) + etas.2.sq / (delta * delta)) / (colSums(Etasq / Lambdas) + etas.2.sq /
                                             delta) - (colSums(1 / Xis) + (n - t) / delta))
    ...
    if (dLL[1] < esp) {
      ...
      optLL <-
        append(
          optLL,
          emma.delta.ML.LL.w.Z(llim, eig.R$values, etas.1, eig.L$values, n, etas.2.sq)
        )
    }
    if (dLL[m - 1] > 0 - esp) {
      optlogdelta <- append(optlogdelta, ulim)
      optLL <-
        append(
          optLL,
          emma.delta.ML.LL.w.Z(ulim, eig.R$values, etas.1, eig.L$values, n, etas.2.sq)
        )
    }
  ...
  if (is.null(Z)) {
    ...
    maxva <-
      (sum(etas.1 * etas.1 / (eig.R$values + maxdelta)) + etas.2.sq / maxdelta) /
      n
  }
...
emma.REMLE <-
  function(y,
    ...
    if (is.null(Z)) {
      ...
      Etasq <- matrix(etas * etas, n - q, m)
      LL <-
        0.5 * ((n - q) * (log((n - q) / (2 * pi)) - 1 - log(colSums(Etasq / Lambdas))) -
                 colSums(log(Lambdas)))
      dLL <-
        0.5 * delta * ((n - q) * colSums(Etasq / (Lambdas * Lambdas)) / colSums(Etasq /
                                                                                  Lambdas) - colSums(1 / Lambdas))
      ...
      if (dLL[1] < esp) {
        ...
        optLL <-
          append(optLL,
                 emma.delta.REML.LL.wo.Z(llim, eig.R$values, etas))
      }
      if (dLL[m - 1] > 0 - esp) {
        optlogdelta <- append(optlogdelta, ulim)
        optLL <-
          append(optLL,
                 emma.delta.REML.LL.wo.Z(ulim, eig.R$values, etas))
      }
      ...
      for (i in 1:(m - 1))
      {
            ...
            (dLL[i] > 0) && (dLL[i + 1] < 0))
        {
          ...
          optLL <-
            append(optLL,
                   emma.delta.REML.LL.wo.Z(r$root, eig.R$values, etas))
        }
      }
      #    optdelta <- exp(optlogdelta)
    }
      ...
      Etasq <- matrix(etas.1 * etas.1, t - q, m)
      dLL <-
        0.5 * delta * ((n - q) * (colSums(Etasq / (
          Lambdas * Lambdas
        )) + etas.2.sq / (delta * delta)) / (colSums(Etasq / Lambdas) + etas.2.sq /
                                               delta) - (colSums(1 / Lambdas) + (n - t) / delta))
      ...
      if (dLL[1] < esp) {
        ...
        optLL <-
          append(optLL,
                 emma.delta.REML.LL.w.Z(llim, eig.R$values, etas.1, n, t, etas.2.sq))
      }
      if (dLL[m - 1] > 0 - esp) {
        optlogdelta <- append(optlogdelta, ulim)
        optLL <-
          append(optLL,
                 emma.delta.REML.LL.w.Z(ulim, eig.R$values, etas.1, n, t, etas.2.sq))
      }
      ...
      for (i in 1:(m - 1))
      {
            ...
            (dLL[i] > 0) && (dLL[i + 1] < 0))
        {
          ...
          optLL <-
            append(optLL,
                   emma.delta.REML.LL.w.Z(r$root, eig.R$values, etas.1, n, t, etas.2.sq))
        }
      }
      #    optdelta <- exp(optlogdelta)
    }
...
emma.ML.LRT <-
  function(ys,
    ...
    if (sum(is.na(ys)) == 0) {
      ...
      for (i in 1:g) {
        ML0[i] <-
          emma.MLE(ys[i, ], X0, K, Z, ngrids, llim, ulim, esp, eig.L, eig.R0)$ML
      }
      ...
      for (i in 1:m) {
        ...
        if ((mean(xv) <= 0) || (mean(xv) >= 1)) {
          ...
          if (is.null(Z)) {
            ...
            nr <- sum(vrows)
            X <-
              cbind(X0[vrows, , drop = FALSE], Z[vrows, vids] %*% t(xs[i, vids, drop =
                                                                         FALSE]))
          ...
          for (j in 1:g) {
            if (nv == t) {
              ...
              if (is.null(Z)) {
                ...
                MLE0 <-
                  emma.MLE(ys[j, vids], X0[vids, , drop = FALSE], K[vids, vids], NULL, ngrids, llim, ulim, esp, eig.L0)
                MLE1 <-
                  emma.MLE(ys[j, vids], X, K[vids, vids], NULL, ngrids, llim, ulim, esp, eig.L0)
              }
              else {
                if (nr == n) {
                  ...
                  MLE0 <-
                    emma.MLE(ys[j, vrows], X0[vrows, , drop = FALSE], K[vids, vids], Z[vrows, vids], ngrids, llim, ulim, esp, eig.L0)
                  MLE1 <-
                    emma.MLE(ys[j, vrows], X, K[vids, vids], Z[vrows, vids], ngrids, llim, ulim, esp, eig.L0)
                }
              }
              if (!ponly) {
                ML1s[i, j] <- MLE1$ML
      ...
      for (i in 1:g) {
        ...
        if (is.null(Z)) {
          ML0[i] <-
            emma.MLE(ys[i, vrows], X0[vrows, , drop = FALSE], K[vrows, vrows], NULL, ngrids, llim, ulim, esp)$ML
        }
        else {
          vids <- colSums(Z[vrows, ] > 0)
          
          ML0[i] <-
            emma.MLE(ys[i, vrows], X0[vrows, , drop = FALSE], K[vids, vids], Z[vrows, vids], ngrids, llim, ulim, esp)$ML
        }
      }
      ...
      for (i in 1:m) {
        ...
        if ((mean(xv) <= 0) || (mean(xv) >= 1)) {
          ...
          if (is.null(Z)) {
            ...
            vrows <- as.logical(rowSums(Z[, vids]))
            X <-
              cbind(X0, Z[, vids, drop = FALSE] %*% t(xs[i, vids, drop = FALSE]))
            if (nv == t) {
            ...
            if (nv == t) {
              ...
              if (is.null(Z)) {
                if (nr == n) {
                  ...
                  MLE <-
                    emma.MLE(ys[j, vrows], X[vrows, ], K[vrows, vrows], NULL, ngrids, llim, ulim, esp)
                }
              }
              else {
                if (nr == n) {
                  ...
                  MLE <-
                    emma.MLE(ys[j, vrows], X[vrows, ], K[vtids, vtids], Z[vrows, vtids], ngrids, llim, ulim, esp)
                }
              }
              ...
              if (is.null(Z)) {
                ...
                MLE0 <-
                  emma.MLE(ys[j, vtids], X0[vtids, , drop = FALSE], K[vtids, vtids], NULL, ngrids, llim, ulim, esp, eig.L0)
                MLE1 <-
                  emma.MLE(ys[j, vtids], X[vtids, ], K[vtids, vtids], NULL, ngrids, llim, ulim, esp, eig.L0)
              }
              else {
                ...
                MLE0 <-
                  emma.MLE(ys[j, vtrows], X0[vtrows, , drop = FALSE], K[vtids, vtids], Z[vtrows, vtids], ngrids, llim, ulim, esp, eig.L0)
                MLE1 <-
                  emma.MLE(ys[j, vtrows], X[vtrows, ], K[vtids, vtids], Z[vtrows, vtids], ngrids, llim, ulim, esp, eig.L0)
              }
              if (!ponly) {
                ML1s[i, j] <- MLE1$ML
                vgs[i, j] <- MLE1$vg
      ...
      for (i in 1:m) {
        ...
        if ((mean(xv) <= 0) || (mean(xv) >= 1)) {
          ...
          if (is.null(Z)) {
            ...
            vrows <- as.logical(rowSums(Z[, vids]))
            X <-
              cbind(X0[vrows, , drop = FALSE], Z[vrows, vids, drop = FALSE] %*% t(xs[i, vids, drop =
                                                                                       FALSE]))
            eig.R1 = emma.eigen.R.w.Z(Z[vrows, vids], K[vids, vids], X)
          }
          ...
          for (j in 1:g) {
            if (nv == t) {
              ...
              if (is.null(Z)) {
                U <-
                  eig.L$vectors * matrix(sqrt(1 / (eig.L$values + REMLE$delta)), t, t, byrow =
                                           TRUE)
                dfs[i, j] <- nv - q1
              }
              else {
              ...
              if (is.null(Z)) {
                ...
                yv <- ys[j, vids]
                REMLE <-
                  emma.REMLE(yv, X, K[vids, vids, drop = FALSE], NULL, ngrids, llim, ulim, esp, eig.R1)
                U <-
                  eig.L0$vectors * matrix(sqrt(1 / (
                    eig.L0$values + REMLE$delta
                  )), nr, nr, byrow = TRUE)
                dfs[i, j] <- nr - q1
              }
              else {
                eig.L0 <-
                  emma.eigen.L.w.Z(Z[vrows, vids, drop = FALSE], K[vids, vids])
                yv <- ys[j, vrows]
                nr <- sum(vrows)
                tv <- sum(vids)
                REMLE <-
                  emma.REMLE(yv, X, K[vids, vids, drop = FALSE], Z[vrows, vids, drop = FALSE], ngrids, llim, ulim, esp, eig.R1)
                U <-
                  eig.L0$vectors * matrix(c(sqrt(
                    1 / (eig.L0$values + REMLE$delta)
                  ), rep(
                    sqrt(1 / REMLE$delta), nr - tv
                  )), nr, nr, byrow = TRUE)
                dfs[i, j] <- nr - q1
            ...
            if (nv == t) {
              ...
              if (is.null(Z)) {
                if (nr == n) {
                  ...
                  REMLE <-
                    emma.REMLE(yv, X[vrows, , drop = FALSE], K[vrows, vrows, drop = FALSE], NULL, ngrids, llim, ulim, esp)
                  U <-
                    eig.L0$vectors * matrix(sqrt(1 / (
                      eig.L0$values + REMLE$delta
                    )), nr, nr, byrow = TRUE)
                ...
                if (nr == n) {
                  ...
                  U <-
                    eig.L$vectors * matrix(c(sqrt(
                      ...
                      sqrt(1 / REMLE$delta), n - t
                    )), n, n, byrow = TRUE)
                }
                else {
                  vtids <- as.logical(colSums(Z[vrows, , drop = FALSE]))
                  eig.L0 <-
                    emma.eigen.L.w.Z(Z[vrows, vtids, drop = FALSE], K[vtids, vtids, drop =
                                                                        FALSE])
                  REMLE <-
                    emma.REMLE(yv, X[vrows, , drop = FALSE], K[vtids, vtids, drop = FALSE], Z[vrows, vtids, drop =
                                                                                                FALSE], ngrids, llim, ulim, esp)
                  U <-
                    eig.L0$vectors * matrix(c(sqrt(
                      1 / (eig.L0$values + REMLE$delta)
                    ), rep(
                      sqrt(1 / REMLE$delta), nr - sum(vtids)
                    )), nr, nr, byrow = TRUE)
                }
                dfs[i, j] <- nr - q1
              }
              ...
              if (is.null(Z)) {
                ...
                nr <- sum(vtids)
                REMLE <-
                  emma.REMLE(yv, X[vtids, , drop = FALSE], K[vtids, vtids, drop = FALSE], NULL, ngrids, llim, ulim, esp)
                U <-
                  eig.L0$vectors * matrix(sqrt(1 / (
                    eig.L0$values + REMLE$delta
                  )), nr, nr, byrow = TRUE)
                ...
                vtrows <-
                  vrows & as.logical(rowSums(Z[, vids, drop = FALSE]))
                eig.L0 <-
                  emma.eigen.L.w.Z(Z[vtrows, vtids, drop = FALSE], K[vtids, vtids, drop =
                                                                       FALSE])
                yv <- ys[j, vtrows]
                nr <- sum(vtrows)
                REMLE <-
                  emma.REMLE(yv, X[vtrows, , drop = FALSE], K[vtids, vtids, drop = FALSE], Z[vtrows, vtids, drop =
                                                                                               FALSE], ngrids, llim, ulim, esp)
                U <-
                  eig.L0$vectors * matrix(c(sqrt(
                    1 / (eig.L0$values + REMLE$delta)
  ...
  function(snps, method = "additive", use = "all") {
    ...
    for (i in 1:(n - 1)) {
      for (j in (i + 1):n) {
        ...
        Num_First_Term_2 <-
          (snps_comp[, i] - mafs_comp[, i]) * (snps_comp[, j] - mafs_comp[, j])
        ...
        Second_Term <-
          sum(Num_Second_Term * Num_Second_Term_Bias_Correction)
...
mlmm <- function(Y, X, K, nbchunks, maxsteps, thresh = NULL) {
  ...
  for (j in 1:(nbchunks - 1)) {
    X_t <-
      crossprod(M %*% (diag(n) - tcrossprod(Q_, Q_)), (X[, !colnames(X) %in% colnames(cof_fwd[[1]])])[, ((j -
                                                                                                            1) * round(m / nbchunks) + 1):(j * round(m / nbchunks))])
    RSS[[j]] <-
      apply(X_t, 2, function(x) {
        sum(lsfit(x, Res_H0, intercept = FALSE)$residuals ^ 2)
      })
    rmx(X_t)
  }
  X_t <-
    crossprod(M %*% (diag(n) - tcrossprod(Q_, Q_)), (X[, !colnames(X) %in% colnames(cof_fwd[[1]])])[, ((j) *
                                                                                                         round(m / nbchunks) + 1):(m - (ncol(cof_fwd[[1]]) - 1))])
  RSS[[nbchunks]] <-
    apply(X_t, 2, function(x) {
      sum(lsfit(x, Res_H0, intercept = FALSE)$residuals ^ 2)
    })
  ...
  Ftest[[2]] <-
    (rep(RSS_H0[[2]], length(RSSf[[2]])) / RSSf[[2]] - 1) * df2[[2]] / df1
  # print(length(RSSf[[2]]))
  ...
  # print(length(pval[[2]]))
  cof_fwd[[2]] <-
    cbind(cof_fwd[[1]], X[, colnames(X) %in% names(which(RSSf[[2]] == min(RSSf[[2]]))[1])])
  colnames(cof_fwd[[2]]) <-
    c(colnames(cof_fwd[[1]]), names(which(RSSf[[2]] == min(RSSf[[2]]))[1]))
  mod_fwd[[2]] <- emma.REMLE(Y, cof_fwd[[2]], K_norm)
  herit_fwd[[2]] <- mod_fwd[[2]]$vg / (mod_fwd[[2]]$vg + mod_fwd[[2]]$ve)
  rmx(M, Y_t, cof_fwd_t, Res_H0, Q_, RSS)
  ...
  for (i in 3:(maxsteps)) {
    if (herit_fwd[[i - 2]] < 0.01)
      ...
      for (j in 1:(nbchunks - 1)) {
        X_t <-
          crossprod(M %*% (diag(n) - tcrossprod(Q_, Q_)), (X[, !colnames(X) %in% colnames(cof_fwd[[i -
                                                                                                     1]])])[, ((j - 1) * round(m / nbchunks) + 1):(j * round(m / nbchunks))])
        RSS[[j]] <-
          apply(X_t, 2, function(x) {
            sum(lsfit(x, Res_H0, intercept = FALSE)$residuals ^ 2)
          })
      ...
      X_t <-
        crossprod(M %*% (diag(n) - tcrossprod(Q_, Q_)), (X[, !colnames(X) %in% colnames(cof_fwd[[i -
                                                                                                   1]])])[, ((j) * round(m / nbchunks) + 1):(m - (ncol(cof_fwd[[i - 1]]) -
                                                                                                                                                    1))])
      RSS[[nbchunks]] <-
        apply(X_t, 2, function(x) {
          sum(lsfit(x, Res_H0, intercept = FALSE)$residuals ^ 2)
      ...
      Ftest[[i]] <-
        (rep(RSS_H0[[i]], length(RSSf[[i]])) / RSSf[[i]] - 1) * df2[[i]] / df1
      pval[[i]] <- pf(Ftest[[i]], df1, df2[[i]], lower.tail = FALSE)
      
      cof_fwd[[i]] <-
        cbind(cof_fwd[[i - 1]], X[, colnames(X) %in% names(which(RSSf[[i]] == min(RSSf[[i]]))[1])])
      colnames(cof_fwd[[i]]) <-
        c(colnames(cof_fwd[[i - 1]]), names(which(RSSf[[i]] == min(RSSf[[i]]))[1]))
  ...
  M <-
    solve(chol(mod_fwd[[length(mod_fwd)]]$vg * K_norm + mod_fwd[[length(mod_fwd)]]$ve *
                 diag(n)))
  Y_t <- crossprod(M, Y)
  ...
  for (j in 1:(nbchunks - 1)) {
    X_t <-
      crossprod(M %*% (diag(n) - tcrossprod(Q_, Q_)), (X[, !colnames(X) %in% colnames(cof_fwd[[length(mod_fwd)]])])[, ((j -
                                                                                                                          1) * round(m / nbchunks) + 1):(j * round(m / nbchunks))])
    RSS[[j]] <-
      apply(X_t, 2, function(x) {
        sum(lsfit(x, Res_H0, intercept = FALSE)$residuals ^ 2)
  ...
  X_t <-
    crossprod(M %*% (diag(n) - tcrossprod(Q_, Q_)), (X[, !colnames(X) %in% colnames(cof_fwd[[length(mod_fwd)]])])[, ((j) *
                                                                                                                       round(m / nbchunks) + 1):(m - (ncol(cof_fwd[[length(mod_fwd)]]) - 1))])
  RSS[[nbchunks]] <-
    apply(X_t, 2, function(x) {
      sum(lsfit(x, Res_H0, intercept = FALSE)$residuals ^ 2)
    })
  ...
  Ftest[[length(mod_fwd) + 1]] <-
    (rep(RSS_H0[[length(mod_fwd) + 1]], length(RSSf[[length(mod_fwd) + 1]])) /
       RSSf[[length(mod_fwd) + 1]] - 1) * df2[[length(mod_fwd) + 1]] / df1
  pval[[length(mod_fwd) + 1]] <-
    pf(Ftest[[length(mod_fwd) + 1]], df1, df2[[length(mod_fwd) + 1]], lower.tail =
         FALSE)
  rmx(M, Y_t, cof_fwd_t, Res_H0, Q_, RSS)
  ...
  max_pval_fwd[1] <- 0
  for (i in 2:length(fwd_lm)) {
    max_pval_fwd[i] <- max(fwd_lm[[i]]$coef[2:i, 4])
  }
  rmx(i)
  ...
  mod_fwd_LL[[1]] <-
    list(nfixed = ncol(cof_fwd[[1]]),
         LL = emma.MLE(Y, cof_fwd[[1]], K_norm)$ML)
  for (i in 2:length(cof_fwd)) {
    mod_fwd_LL[[i]] <-
      list(nfixed = ncol(cof_fwd[[i]]),
           LL = emma.MLE(Y, cof_fwd[[i]], K_norm)$ML)
  }
  rmx(i)
  ...
  dropcof_bwd[[1]] <- 'NA'
  cof_bwd[[1]] <-
    as.matrix(cof_fwd[[length(mod_fwd)]][, !colnames(cof_fwd[[length(mod_fwd)]]) %in% dropcof_bwd[[1]]])
  colnames(cof_bwd[[1]]) <-
    colnames(cof_fwd[[length(mod_fwd)]])[!colnames(cof_fwd[[length(mod_fwd)]]) %in% dropcof_bwd[[1]]]
  ...
  for (i in 2:length(mod_fwd)) {
    dropcof_bwd[[i]] <-
      (colnames(cof_bwd[[i - 1]])[2:ncol(cof_bwd[[i - 1]])])[which(abs(bwd_lm[[i -
                                                                                 1]]$coef[2:nrow(bwd_lm[[i - 1]]$coef), 3]) == min(abs(bwd_lm[[i - 1]]$coef[2:nrow(bwd_lm[[i -
                                                                                                                                                                             1]]$coef), 3])))]
    cof_bwd[[i]] <-
      as.matrix(cof_bwd[[i - 1]][, !colnames(cof_bwd[[i - 1]]) %in% dropcof_bwd[[i]]])
    colnames(cof_bwd[[i]]) <-
      colnames(cof_bwd[[i - 1]])[!colnames(cof_bwd[[i - 1]]) %in% dropcof_bwd[[i]]]
  ...
  for (i in 1:(length(bwd_lm) - 1)) {
    max_pval_bwd[i] <- max(bwd_lm[[i]]$coef[2:(length(bwd_lm) + 1 - i), 4])
  }
  ...
  mod_bwd_LL <- list()
  mod_bwd_LL[[1]] <-
    list(nfixed = ncol(cof_bwd[[1]]),
         LL = emma.MLE(Y, cof_bwd[[1]], K_norm)$ML)
  for (i in 2:length(cof_bwd)) {
    mod_bwd_LL[[i]] <-
      list(nfixed = ncol(cof_bwd[[i]]),
           LL = emma.MLE(Y, cof_bwd[[i]], K_norm)$ML)
  }
  rmx(i)
  ...
  BIC <- function(x) {
    -2 * x$LL + (x$nfixed + 1) * log(n)
  }
  extBIC <- function(x) {
    BIC(x) + 2 * lchoose(m, x$nfixed - 1)
  }
  # print(ncol(cof_fwd[[1]]))
  fwd_table <-
    data.frame(
      step = ncol(cof_fwd[[1]]) - 1,
      step_ = paste('fwd', ncol(cof_fwd[[1]]) - 1, sep = ''),
      cof = 'NA',
      ncof = ncol(cof_fwd[[1]]) - 1,
      h2 = herit_fwd[[1]]
      ,
      maxpval = max_pval_fwd[1],
      BIC = BIC(mod_fwd_LL[[1]]),
      extBIC = extBIC(mod_fwd_LL[[1]])
    )
  for (i in 2:(length(mod_fwd))) {
    fwd_table <- rbind(
      fwd_table,
      data.frame(
        step = ncol(cof_fwd[[i]]) - 1,
        step_ = paste('fwd', ncol(cof_fwd[[i]]) - 1, sep = ''),
        cof = paste('+', colnames(cof_fwd[[i]])[i], sep = ''),
        ncof = ncol(cof_fwd[[i]]) - 1,
        h2 = herit_fwd[[i]]
        ,
        maxpval = max_pval_fwd[i],
        BIC = BIC(mod_fwd_LL[[i]]),
        extBIC = extBIC(mod_fwd_LL[[i]])
      )
    )
  }
  ...
  bwd_table <-
    data.frame(
      step = length(mod_fwd),
      step_ = paste('bwd', 0, sep = ''),
      cof = paste('-', dropcof_bwd[[1]], sep = ''),
      ncof = ncol(cof_bwd[[1]]) - 1,
      h2 = herit_bwd[[1]]
      ,
      maxpval = max_pval_bwd[1],
      BIC = BIC(mod_bwd_LL[[1]]),
      extBIC = extBIC(mod_bwd_LL[[1]])
    )
  for (i in 2:(length(mod_bwd))) {
    bwd_table <- rbind(
      bwd_table,
      data.frame(
        step = length(mod_fwd) + i - 1,
        step_ = paste('bwd', i - 1, sep = ''),
        cof = paste('-', dropcof_bwd[[i]], sep = ''),
        ncof = ncol(cof_bwd[[i]]) - 1,
        h2 = herit_bwd[[i]]
        ,
        maxpval = max_pval_bwd[i],
        BIC = BIC(mod_bwd_LL[[i]]),
        extBIC = extBIC(mod_bwd_LL[[i]])
      )
    )
  }
  
  rmx(i, BIC, extBIC, max_pval_fwd, max_pval_bwd, dropcof_bwd)
  ...
  for (i in 2:length(mod_fwd)) {
    mod_fwd_RSS[i] <- sum((Y - cof_fwd[[i]] %*% fwd_lm[[i]]$coef[, 1]) ^ 2)
  }
  mod_bwd_RSS <- vector()
  mod_bwd_RSS[1] <- sum((Y - cof_bwd[[1]] %*% bwd_lm[[1]]$coef[, 1]) ^ 2)
  for (i in 2:length(mod_bwd)) {
    mod_bwd_RSS[i] <- sum((Y - cof_bwd[[i]] %*% bwd_lm[[i]]$coef[, 1]) ^ 2)
  }
  expl_RSS <-
    c(1 - sapply(mod_fwd_RSS, function(x) {
      x / mod_fwd_RSS[1]
    }),
    1 - sapply(mod_bwd_RSS, function(x) {
      x / mod_bwd_RSS[length(mod_bwd_RSS)]
    }))
  ...
  for (i in 2:(length(mod_fwd))) {
    pval_step[[i]] <-
      list(
        out = rbind(
          data.frame(SNP = colnames(cof_fwd[[i]])[-1], 'pval' = fwd_lm[[i]]$coef[2:i, 4]),
          data.frame(SNP = colnames(X)[-which(colnames(X) %in% colnames(cof_fwd[[i]]))], 'pval' =
                       pval[[i + 1]])
        ),
        "cof" = colnames(cof_fwd[[i]])[-1],
        "coef" = fwd_lm[[i]]$coef
      )
  }
  ...
  opt_extBIC <-
    fwdbwd_table[which(fwdbwd_table$extBIC == min(fwdbwd_table$extBIC))[1], ]
  opt_mbonf <-
    (fwdbwd_table[which(fwdbwd_table$maxpval <= 0.05 / m), ])[which(fwdbwd_table[which(fwdbwd_table$maxpval <=
                                                                                         0.05 / m), ]$ncof == max(fwdbwd_table[which(fwdbwd_table$maxpval <= 0.05 /
                                                                                                                                       m), ]$ncof))[1], ]
  if (!is.null(thresh)) {
    opt_thresh <-
      (fwdbwd_table[which(fwdbwd_table$maxpval <= thresh), ])[which(fwdbwd_table[which(fwdbwd_table$maxpval <=
                                                                                         thresh), ]$ncof == max(fwdbwd_table[which(fwdbwd_table$maxpval <= thresh), ]$ncof))[1], ]
  }
  bestmodel_pvals <- function(model) {
    ...
    if (substr(model$step_, start = 0, stop = 3) == 'fwd') {
      ...
      for (j in 1:(nbchunks - 1)) {
        X_t <-
          crossprod(M %*% (diag(n) - tcrossprod(Q_, Q_)), (X[, !colnames(X) %in% colnames(cof)])[, ((j -
                                                                                                       1) * round(m / nbchunks) + 1):(j * round(m / nbchunks))])
        RSS[[j]] <-
          apply(X_t, 2, function(x) {
            sum(lsfit(x, Res_H0, intercept = FALSE)$residuals ^ 2)
      ...
      X_t <-
        crossprod(M %*% (diag(n) - tcrossprod(Q_, Q_)), (X[, !colnames(X) %in% colnames(cof)])[, ((j) *
                                                                                                    round(m / nbchunks) + 1):(m - (ncol(cof) - 1))])
      RSS[[nbchunks]] <-
        apply(X_t, 2, function(x) {
          sum(lsfit(x, Res_H0, intercept = FALSE)$residuals ^ 2)
        })
      ...
      list(
        'out' = rbind(
          data.frame(SNP = colnames(cof)[-1], 'pval' = GLS_lm$coef[2:(ncol(cof)), 4]),
          data.frame('SNP' = colnames(X)[-which(colnames(X) %in% colnames(cof))], 'pval' =
                       pval)
        ),
        'cof' = colnames(cof)[-1],
        'coef' = GLS_lm$coef
  ...
  output <-
    list(
      step_table = fwdbwd_table,
      pval_step = pval_step,
      RSSout = plot_RSS,
      bonf_thresh = -log10(0.05 / m),
      opt_extBIC = opt_extBIC_out,
      opt_mbonf = opt_mbonf_out,
      seqQTN = seqQTN
    )
  if (!is.null(thresh)) {
...
mlmm_cof <- function(Y, X, cofs, K, nbchunks, maxsteps, thresh = NULL) {
  ...
  for (j in 1:(nbchunks - 1)) {
    X_t <-
      crossprod(M %*% (diag(n) - tcrossprod(Q_, Q_)), (X[, !colnames(X) %in% addcof_fwd[[1]]])[, ((j -
                                                                                                     1) * round(m / nbchunks) + 1):(j * round(m / nbchunks))])
    RSS[[j]] <-
      apply(X_t, 2, function(x) {
        sum(lsfit(x, Res_H0, intercept = FALSE)$residuals ^ 2)
  ...
  X_t <-
    crossprod(M %*% (diag(n) - tcrossprod(Q_, Q_)), (X[, !colnames(X) %in% addcof_fwd[[1]]])[, ((j) *
                                                                                                  round(m / nbchunks) + 1):(m - (ncol(cof_fwd[[1]])))])
  RSS[[nbchunks]] <-
    apply(X_t, 2, function(x) {
      sum(lsfit(x, Res_H0, intercept = FALSE)$residuals ^ 2)
    })
  ...
  cof_fwd[[2]] <-
    cbind(cof_fwd[[1]], X[, colnames(X) %in% addcof_fwd[[2]]])
  ...
  for (i in 3:(maxsteps)) {
    if (herit_fwd[[i - 2]] < 0.01)
      ...
      cof_fwd[[i]] <-
        cbind(cof_fwd[[i - 1]], X[, colnames(X) %in% addcof_fwd[[i]]])
  ...
  X_t <-
    crossprod(M %*% (diag(n) - tcrossprod(Q_, Q_)), (X[, !colnames(X) %in% colnames(cof_fwd[[length(mod_fwd)]])])[, ((j) *
                                                                                                                       round(m / nbchunks) + 1):(m - (ncol(cof_fwd[[length(mod_fwd)]])))])
  RSS[[nbchunks]] <-
    apply(X_t, 2, function(x) {
      sum(lsfit(x, Res_H0, intercept = FALSE)$residuals ^ 2)
    })
  ...
  RSSf[[length(mod_fwd) + 1]] <- unlist(RSS)
  RSS_H0[[length(mod_fwd) + 1]] <- sum(Res_H0 ^ 2)
  df2[[length(mod_fwd) + 1]] <-
    n - df1 - ncol(fix_cofs) - ncol(cof_fwd[[length(mod_fwd)]])
  Ftest[[length(mod_fwd) + 1]] <-
    (rep(RSS_H0[[length(mod_fwd) + 1]], length(RSSf[[length(mod_fwd) + 1]])) /
       RSSf[[length(mod_fwd) + 1]] - 1) * df2[[length(mod_fwd) + 1]] / df1
  pval[[length(mod_fwd) + 1]] <-
  ...
  for (i in 2:length(fwd_lm)) {
    max_pval_fwd[i] <-
      max(fwd_lm[[i]]$coef[(ncol(fix_cofs) + 1):(ncol(fix_cofs) + ncol(cof_fwd[[i]])), 4])
  }
  rmx(i)
  ...
  mod_fwd_LL <- list()
  mod_fwd_LL[[1]] <-
    list(nfixed = ncol(cbind(fix_cofs, cof_fwd[[1]])),
         LL = emma.MLE(Y, cbind(fix_cofs, cof_fwd[[1]]), K_norm)$ML)
  for (i in 2:length(cof_fwd)) {
    mod_fwd_LL[[i]] <-
      list(nfixed = ncol(cbind(fix_cofs, cof_fwd[[i]])),
           LL = emma.MLE(Y, cbind(fix_cofs, cof_fwd[[i]]), K_norm)$ML)
  }
  rmx(i)
  ...
  for (i in 2:length(mod_fwd)) {
    dropcof_bwd[[i]] <-
      colnames(cof_bwd[[i - 1]])[which(abs(bwd_lm[[i - 1]]$coef[(ncol(fix_cofs) +
                                                                   1):nrow(bwd_lm[[i - 1]]$coef), 3]) == min(abs(bwd_lm[[i - 1]]$coef[(ncol(fix_cofs) +
                                                                                                                                         1):nrow(bwd_lm[[i - 1]]$coef), 3])))]
    cof_bwd[[i]] <-
      as.matrix(cof_bwd[[i - 1]][, !colnames(cof_bwd[[i - 1]]) %in% dropcof_bwd[[i]]])
    colnames(cof_bwd[[i]]) <-
      colnames(cof_bwd[[i - 1]])[!colnames(cof_bwd[[i - 1]]) %in% dropcof_bwd[[i]]]
  ...
  for (i in 1:(length(bwd_lm) - 1)) {
    max_pval_bwd[i] <-
      max(bwd_lm[[i]]$coef[(ncol(fix_cofs) + 1):(ncol(fix_cofs) + ncol(cof_bwd[[i]])), 4])
  }
  max_pval_bwd[length(bwd_lm)] <- 0
  ...
  mod_bwd_LL <- list()
  mod_bwd_LL[[1]] <-
    list(nfixed = ncol(cbind(fix_cofs, cof_bwd[[1]])),
         LL = emma.MLE(Y, cbind(fix_cofs, cof_bwd[[1]]), K_norm)$ML)
  for (i in 2:length(cof_bwd)) {
    mod_bwd_LL[[i]] <-
      list(nfixed = ncol(cbind(fix_cofs, cof_bwd[[i]])),
           LL = emma.MLE(Y, cbind(fix_cofs, cof_bwd[[i]]), K_norm)$ML)
  }
  rmx(i)
  ...
  fwd_table <-
    data.frame(
      step = ncol(cof_fwd[[1]]),
      step_ = paste('fwd', ncol(cof_fwd[[1]]), sep = ''),
      cof = paste('+', addcof_fwd[[1]], sep = ''),
      ncof = ncol(cof_fwd[[1]]),
      h2 = herit_fwd[[1]]
      ,
      maxpval = max_pval_fwd[1],
  ...
  for (i in 2:(length(mod_fwd))) {
    fwd_table <- rbind(
      ...
      data.frame(
        step = ncol(cof_fwd[[i]]),
        step_ = paste('fwd', ncol(cof_fwd[[i]]), sep = ''),
        cof = paste('+', addcof_fwd[[i]], sep = ''),
        ncof = ncol(cof_fwd[[i]]),
        h2 = herit_fwd[[i]]
        ,
        maxpval = max_pval_fwd[i],
  ...
  mod_fwd_RSS <- vector()
  mod_fwd_RSS[1] <-
    sum((Y - cbind(fix_cofs, cof_fwd[[1]]) %*% fwd_lm[[1]]$coef[, 1]) ^ 2)
  for (i in 2:length(mod_fwd)) {
    mod_fwd_RSS[i] <-
      sum((Y - cbind(fix_cofs, cof_fwd[[i]]) %*% fwd_lm[[i]]$coef[, 1]) ^ 2)
  }
  mod_bwd_RSS <- vector()
  mod_bwd_RSS[1] <-
    sum((Y - cbind(fix_cofs, cof_bwd[[1]]) %*% bwd_lm[[1]]$coef[, 1]) ^ 2)
  for (i in 2:length(mod_bwd)) {
    mod_bwd_RSS[i] <-
      sum((Y - cbind(fix_cofs, cof_bwd[[i]]) %*% bwd_lm[[i]]$coef[, 1]) ^ 2)
  }
  ...
  plot_RSS <-
    t(apply(cbind(
      fix_cofs_RSS, cofs_RSS, h2_RSS, unexpl_RSS
    ), 1, cumsum))
  ...
  pval_step[[1]] <-
    list(
      out = data.frame('SNP' = names(pval[[2]]), 'pval' = pval[[2]]),
      cof = addcof_fwd[[1]],
      "coef" = fwd_lm[[1]]$coef
    )
  for (i in 2:(length(mod_fwd))) {
    pval_step[[i]] <-
      list(
        'out' = rbind(
          data.frame('SNP' = colnames(cof_fwd[[i]]), 'pval' = fwd_lm[[i]]$coef[(ncol(fix_cofs) +
                                                                                  1):(ncol(fix_cofs) + ncol(cof_fwd[[i]])), 4]),
          data.frame('SNP' = names(pval[[i + 1]]), 'pval' =
                       pval[[i + 1]])
        ),
  ...
  bestmodel_pvals <-
    function(model) {
      if (substr(model$step_, start = 0, stop = 3) == 'fwd') {
        ...
        list(
          'out' = rbind(
            data.frame(SNP = colnames(cof), 'pval' = GLS_lm$coef[(ncol(fix_cofs) + 1):(ncol(fix_cofs) +
                                                                                         ncol(cof)), 4]),
  ...
  function(Y = NULL,
    ...
    if (is.null(Y) &
      ...
      return (
        list(
          ...
          compression = NULL,
          kinship.optimum = NULL,
          kinship = myGenotype$KI,
          PCA = myGenotype$PC,
          GD = data.frame(cbind(as.data.frame(GT), as.data.frame(GD))),
          GI = GI,
          G = myGenotype$G
        )
      )
    ...
    if (is.null(Y))
      return (
        list(
          ...
          kinship = myGenotype$KI,
          PCA = myGenotype$PC,
          GD = data.frame(cbind(as.date.frame(GT), as.data.frame(GD))),
          Gi = GI,
          G = myGenotype$G
        )
      )
    ...
    if (!is.null(Y)) {
      ...
      for (trait in 2:ncol(Y))  {
        ...
        ###Correlation between phenotype and principal components
        if (!is.null(Y) &
            !is.null(PC) & file.output & PCA.total > 0 & PCA.View.output) {
          myPPV <- GAPIT.Phenotype.PCA.View(PC = PC,
                                            myY = Y[, c(1, trait)])
        ...
        print(paste("Processing trait: ", traitname, sep = ""))
        if (!is.null(memo))
          traitname = paste(memo, ".", traitname, sep = "")
      ...
      if (ncol(Y) == 2) {
        if (!SUPER_GS) {
          ...
          h2 = as.matrix(
            as.numeric(as.vector(gapitMain$Compression[, 5])) / (as.numeric(
              as.vector(gapitMain$Compression[, 5])
            ) + as.numeric(
              as.vector(gapitMain$Compression[, 6])
            )),
            length(gapitMain$Compression[, 6]),
            1
          )
          colnames(h2) = c("Heritability")
          print(
            "GAPIT accomplished successfully for single trait. Results are saved. GWAS are returned!"
          )
          print(
            "It is OK to see this: 'There were 50 or more warnings (use warnings() to see the first 50)'"
          )
          ...
          return (
            list(
              ...
              h2 = gapitMain$h2,
              Pred = gapitMain$Pred,
              compression = as.data.frame(cbind(gapitMain$Compression, h2)),
              kinship.optimum = gapitMain$kinship.optimum,
              kinship = gapitMain$kinship,
              PCA = gapitMain$PC,
              FDR = myPower$FDR,
              Power = myPower$Power,
              Power.Alpha = myPower$Power.Alpha,
              ...
              P = gapitMain$P,
              effect.snp = gapitMain$effect.snp,
              effect.cv = gapitMain$effect.cv
            )
          )
        } else{
          h2 = as.matrix(
          ...
          colnames(h2) = c("Heritability")
          print(
            "GAPIT accomplished successfully for single trait. Results are saved. GPS are returned!"
          )
          print(
            "It is OK to see this: 'There were 50 or more warnings (use warnings() to see the first 50)'"
          )
