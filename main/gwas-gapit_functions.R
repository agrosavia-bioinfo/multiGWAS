...
#-----------------------------------------------------------------------
# Create by LG to renamed rm base fuction to avoid warnmings
#-----------------------------------------------------------------------
rm <- function (...) {
          ...
          if(sum(threshold!=0)==length(threshold)){
            for(thr in 1:length(threshold)){
              ...
              if(significantline1<H){
                ...
                message(paste("No significant points for ",taxa[i]," pass the threshold level using threshold=",threshold[thr],"!",sep=""))
              }
            }
          }
        }
            ...
            for (i in loopStart:mloop){
              ...
              if(normalCase)
              {
                ...
                if(i ==0 &file==file.from &frag==1 & !is.null(K))
                {
				  ...
				  # LG change is.na to anyNA as gives more than one results
                  #if(!is.na(CVI)){
                  if(!anyNA(CVI)){
...
`FarmCPU.BIN` <-
  function(Y=NULL,GDP=NULL,GM=NULL,CV=NULL,P=NULL,orientation="col",method="random",b=c(5e5,5e6,5e7),s=seq(10,100,10),theLoop=NULL,bound=NULL){
    ...
    if(!optimumable & method=="optimum"){
      #print("message: method was changed from optimum to static")
      method="static"
    }
...
`FarmCPU` <-
  function(Y=NULL,GD=NULL,GM=NULL,CV=NULL,GP=NULL,Yt=NULL,DPP=1000000,kinship.algorithm="FARM-CPU",file.output=TRUE,cutOff=0.01,method.GLM="FarmCPU.LM",method.sub="reward",method.sub.final="reward",method.bin="static",bin.size=c(5e5,5e6,5e7),bin.selection=seq(10,100,10),
    ...
    index=which(GM[,3]==0 )
    if(length(index)>0){
      #print("message: there is 0 bp which was set to 1")
      #print(length(index))
...
`GAPIT.GS` <-
  function(KW,KO,KWO,GAU,UW){
    ...
    if(inherits(UO, "try-error")) UO=t(KWO)%*%MASS::ginv (KW)%*%UW
...
`GAPIT.GS.Visualization` <-
  function(gsBLUP = gsBLUP, BINS=BINS, name.of.trait = name.of.trait){
    ...
    if(BLUP[1]=="NaN"){
      message ("It was not converged. BLUP was not created!")
    }
    if(BLUP[1]!="NaN" )
    {
...
`GAPIT.Genotype` <-
  function(G=NULL,GD=NULL,GM=NULL,KI=NULL,
    ...
    if(!is.null(GLD) &file.output)
    {
      ...
      {
        GLD=GLD[1,]
        print("message: The number of SNPs requested is beyond limitation. No LD plot created.")
      }
      if(nrow(GLD)>1)
      {
        print("Plot LD...")
        ...
        if(!inherits(MyHeatmap, "try-error")) 
        {
          ...
          print("message: error in converting genotype. No LD plot!")
        }
        dev.off()
        print("LD heatmap crated")
      }else{ # alternative of if(nrow(GLD)>1)
        print("message: There are less than two SNPs on the region you sepcified. No LD plot!")
...
`GAPIT.Memory` <-
  function(Memory =NULL,Infor){
    ...
    ##############################################################################################
    gc()
    # Modified by LG to avoid warning: 'memory.size()' is Windows-specific
...
`GAPIT.Multiple.Manhattan` <-
  function(model_store,DPP=50000,cutOff=0.01,band=5,seqQTN=NULL,Y=NULL,GM=NULL,interQTN=NULL,plot.style="Oceanic",plot.line=FALSE){
    ...
    # print("!!!!")
    # print(result0)
    pdf(paste("GAPIT.Manhattan.Mutiple.Plot.wide.",colnames(result0)[-c(1:3)],".pdf" ,sep = ""), width = 16,height=8.5)
    par(mfrow=c(Nenviron,1))
    for(k in 1:Nenviron)
    { if(k==Nenviron){#par(mfrow=c(Nenviron,1))
      par(mar = c(3,8,1,8))
        ...
        for (i in 1:m)
        {
          ...
          if(min(xs[,i])!=max(xs[,i]))
          {
            ...
            if(length(index.k)<2){
              ...
              #print("message: there is a snp correlated with all QTNs")
            }   
